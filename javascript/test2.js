...
// ==========================================
const SUPABASE_URL = isProd ? 'https://wmxrzjmadvivvpzbslgj.supabase.co' : 'https://wmxrzjmadvivvpzbslgj.supabase.co';
const SUPABASE_KEY = isProd ? 'sb_publishable_Hohs2ojpVd5nmRJoi0upNg_PJv8M7x6' : 'sb_publishable_Hohs2ojpVd5nmRJoi0upNg_PJv8M7x6';

const supabasedb = supabasedb.createClient(SUPABASE_URL, SUPABASE_KEY);
...
// input: none
// ****************************************** 
// Fetches initial data from Supabase and sets up real-time listener.
// ****************************************** 
async function init() {
    // Initial Fetch
    ...
    if (error) return console.error("Error fetching characters:", error);
    ...
    supabasedb
        ...
        .on('postgres_changes', { event: '*', schema: 'public', table: 'characters' }, async () => {
            const { data: updatedData } = await supabasedb.from('characters').select('*');
...
async function saveCharacter() {
    ...
    let charObj;
    if (editIndex > -1) {
        charObj = { ...characters[editIndex], name, group, slug, complexity };
    } else {
        charObj = {
            name, 
            group, 
    ...
    const { error } = await supabasedb.from('characters').upsert(charObj);
    if (error) alert("Error saving hero: " + error.message);
    resetForm();
}
...
async function applyResults() {
    ...
    characters.forEach(char => {
        ...
        [0, 1, 2, 3].forEach(pIdx => {
            ...
            if (playerChoice !== undefined) {
                ...
                if (wasPicked) {
                    if (!char.play_count) char.play_count = [0, 0, 0, 0];
                    if (!char.last_played) char.last_played = ["", "", "", ""];

                    char.play_count[pIdx] = (char.play_count[pIdx] || 0) + 1;
                    char.last_played[pIdx] = today;
                }
            }
        });
        updates.push(char);
    ...
    const { error } = await supabasedb.from('characters').upsert(updates);
    if (error) return alert("Error updating session: " + error.message);
...
async function resetAllWeights() {
    if (confirm("Reset all probabilities to 100?")) {
        ...
        const { error } = await supabasedb.from('characters').upsert(updates);
        if (error) alert("Error resetting weights: " + error.message);
    }
}
