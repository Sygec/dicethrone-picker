(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=window.location.hostname===`sygec.github.io`||window.location.hostname===`dicethrone-prod.sygec.workers.dev`,t=`https://ojqkkixtvdtccuixishh.supabase.co`,n=`sb_publishable_AT9BZrEkq1IDrZmP1Y_pDQ_Qwnh57ZH`,r=`https://wmxrzjmadvivvpzbslgj.supabase.co`,i=`sb_publishable_Hohs2ojpVd5nmRJoi0upNg_PJv8M7x6`,a=e?t:r,o=e?n:i,s=supabase.createClient(a,o);async function c({email:e,password:t}){return s.auth.signInWithPassword({email:e,password:t})}async function l(e){return s.auth.resetPasswordForEmail(e,{redirectTo:window.location.origin})}async function u({password:e}){return s.auth.updateUser({password:e})}async function d(){return s.auth.signOut()}async function f(){return s.auth.getSession()}function p(e){return s.auth.onAuthStateChange(e)}async function m(){return s.from(`groups`).select(`*`).eq(`is_active`,!0).order(`order_index`,{ascending:!0})}async function h(){let e=await s.from(`players`).select(`*`);return e.data&&e.data.sort((e,t)=>parseInt(e.id.slice(1),10)-parseInt(t.id.slice(1),10)),e}async function g(){return s.from(`heroes`).select(`
        ...
        `).order(`played_at`,{ascending:!1}).order(`player_id`,{foreignTable:`game_players`,ascending:!0})}async function v(){return s.from(`user_heroes`).select(`*`)}async function y(e,t){return s.from(`players`).update({player_color:t}).eq(`id`,e).select().single()}async function b(e,t){return s.from(`players`).update({name:t}).eq(`id`,e).select().single()}async function x(e,t,n){return s.from(`user_heroes`).upsert({user_id:e,hero_id:t,is_owned:n})}async function S(e){return s.from(`user_heroes`).upsert(e)}async function ee(e){return s.from(`heroes`).insert(e).select().single()}async function te(e){return s.from(`heroes`).upsert(e).select().single()}async function ne(e){return s.from(`heroes`).delete().eq(`id`,e)}async function C(e){return s.from(`groups`).upsert(e).select().single()}async function w(e){return s.from(`groups`).delete().eq(`id`,e)}async function T(e,t){return s.from(`games`).insert({last_updated_by:e,game_type:t}).select().single()}async function re(e){return s.from(`teams`).insert(e).select()}async function ie(e){return s.from(`game_players`).insert(e)}async function ae(e){return s.from(`player_hero_stats`).upsert(e)}async function oe(e,t,n){if(t===`draw`)return s.from(`game_players`).update({is_winner:!1,last_updated_by:n}).eq(`game_id`,e);{let r=await s.from(`game_players`).update({is_winner:!0,last_updated_by:n}).eq(`game_id`,e).eq(`player_id`,t);return r.error?r:s.from(`game_players`).update({is_winner:!1,last_updated_by:n}).eq(`game_id`,e).neq(`player_id`,t)}}async function se(e,t,n){if(t===`draw`){let t=await s.from(`teams`).update({is_winner:!1,last_updated_by:n}).eq(`game_id`,e);return t.error?t:s.from(`game_players`).update({is_winner:!1,last_updated_by:n}).eq(`game_id`,e)}let{data:r,error:i}=await s.from(`teams`).select(`id, team_label`).eq(`game_id`,e);if(i)return{error:i};let a=r.find(e=>e.team_label===t),o=r.find(e=>e.team_label!==t);if(!a||!o)return{error:Error(`Could not resolve both teams for this game.`)};let c=await s.from(`teams`).update({is_winner:!0,last_updated_by:n}).eq(`id`,a.id);if(c.error)return c;let l=await s.from(`teams`).update({is_winner:!1,last_updated_by:n}).eq(`id`,o.id);if(l.error)return l;let u=await s.from(`game_players`).update({is_winner:!0,last_updated_by:n}).eq(`game_id`,e).eq(`team_id`,a.id);return u.error?u:s.from(`game_players`).update({is_winner:!1,last_updated_by:n}).eq(`game_id`,e).eq(`team_id`,o.id)}async function ce(e){return s.from(`games`).delete().eq(`id`,e)}function le(e){return s.channel(`schema-db-changes`).on(`postgres_changes`,{event:`*`,schema:`public`,table:`user_heroes`},e).on(`postgres_changes`,{event:`*`,schema:`public`,table:`player_hero_stats`},e).on(`postgres_changes`,{event:`*`,schema:`public`,table:`heroes`},e).on(`postgres_changes`,{event:`*`,schema:`public`,table:`games`},e).on(`postgres_changes`,{event:`*`,schema:`public`,table:`game_players`},e).subscribe()}var ue={NAMES:[],characters:[],games:[],players:[],invitees:[],selectedGameType:null,rollModeChosen:!1,teamAssignments:null,teamSwapSource:null,groups:[],authUsers:[],cachedChangelog:null,activeLevels:new Set([1,2,3,4,5,6]),activeGroups:new Set,selectedGamePlayerIndex:null,expandedGameIds:new Set,currentSort:`name`,sortAsc:!0,currentSortPlayerIndex:0,editIndex:-1,activePlayerIndices:[0,1,2,3],currentDrawerMode:`sort-filter`,stagedSort:``,stagedSortAsc:!0,stagedSortPlayerIndex:0,stagedLevels:new Set,stagedGroups:new Set,stagedPlayerIndices:[],stagedUseHistorical:!0,dbUseHistorical:!0,activeFilterDataHistories:new Set,activeFilterPlayers:new Set,activeFilterComplexities:new Set,activeFilterGroups:new Set,stagedFilterDataHistories:new Set,stagedFilterPlayers:new Set,stagedFilterComplexities:new Set,stagedFilterGroups:new Set,activeOwnershipFilter:`owned`,stagedOwnershipFilter:`owned`,gamesWinnerOnly:!1,gamesUseHistorical:!0,stagedSelectedGamePlayerIndex:null,stagedGamesWinnerOnly:!1,stagedGamesUseHistorical:!0,currentUser:null,loggedInPlayerIndex:-1,isRollActive:!1,expandedCollectionGroups:new Set,scrambleIntervals:{},activeSelectPlayerIdx:null,modalSortMode:`name`,draftModeEnabled:!1,draftCount:3,draftCountChosen:!1,bannedHeroIds:new Set,stagedBannedHeroIds:new Set,stagedBanSearchQuery:``,activeDraftOrder:[],activeDraftStep:0,selectedDraftHeroes:{},activeDraftCandidates:{},activeRollParticipants:[],gamesHistoryStyle:`gorgeous`},de=new Set;function fe(e){return de.add(e),()=>{de.delete(e)}}function pe(e,t){de.forEach(n=>{try{n(e,t,ue)}catch(e){console.error(`Error in stateStore listener:`,e)}})}function E(e){return ue[e]}function D(e,t){ue[e]=t,pe(e,t)}function O(e,t,n){let r=ue[e];if(!(r instanceof Set)){console.warn(`stateStore: ${e} is not an instance of Set.`);return}t===`add`?r.add(n):t===`delete`?r.delete(n):t===`clear`?r.clear():t===`toggle`&&(r.has(n)?r.delete(n):r.add(n)),pe(e,r)}function me(e,t,n){let r=ue[e];if(typeof r!=`object`||!r){console.warn(`stateStore: ${e} is not an object.`);return}n===void 0?delete r[t]:r[t]=n,pe(e,r)}function he(e,t=`info`){let n=document.getElementById(`toast-container`);n||(n=document.createElement(`div`),n.id=`toast-container`,document.body.appendChild(n));let r=document.createElement(`div`);r.className=`toast toast-${t}`,r.innerHTML=`
        <span class="toast-message"></span>
        <button class="toast-close" aria-label="Close">&times;</button>
    `,r.querySelector(`.toast-message`).textContent=e,n.appendChild(r),requestAnimationFrame(()=>{r.classList.add(`show`)});let i=()=>{r.parentNode&&(r.classList.remove(`show`),r.addEventListener(`transitionend`,()=>{r.parentNode&&r.parentNode.removeChild(r)}))},a=setTimeout(i,4e3);r.querySelector(`.toast-close`).addEventListener(`click`,()=>{clearTimeout(a),i()})}function k(e,t){return new Promise(n=>{let r=document.createElement(`div`);r.id=`confirm-modal-overlay`,r.innerHTML=`
        ...
        `,r.querySelector(`.confirm-modal-title`).textContent=e,r.querySelector(`.confirm-modal-message`).textContent=t,document.body.appendChild(r),requestAnimationFrame(()=>{r.classList.add(`show`)});let i=()=>{r.classList.remove(`show`),r.addEventListener(`transitionend`,()=>{r.parentNode&&r.parentNode.removeChild(r)})};r.querySelector(`#confirm-cancel-btn`).addEventListener(`click`,()=>{i(),n(!1)}),r.querySelector(`#confirm-confirm-btn`).addEventListener(`click`,()=>{i(),n(!0)}),r.addEventListener(`click`,e=>{e.target===r&&(i(),n(!1))})})}var ge=null,A=()=>(ge||={adminNav:document.querySelector(`.bottom-nav .admin-only`),authBtn:document.getElementById(`auth-btn`),actionButtons:document.getElementById(`action-buttons`),avatarBtn:document.getElementById(`header-avatar-btn`),accountModal:document.getElementById(`account-modal`),loginModal:document.getElementById(`login-modal`),loginError:document.getElementById(`login-error`),loginEmailInput:document.getElementById(`login-email`),updatePasswordModal:document.getElementById(`update-password-modal`),updatePasswordError:document.getElementById(`update-password-error`),updatePasswordUsername:document.getElementById(`update-password-username`),newPasswordInput:document.getElementById(`new-password`),confirmPasswordInput:document.getElementById(`confirm-password`),playerTogglesContainer:document.getElementById(`player-toggle-zone-top`)},ge);function _e(){let e=A(),t=E(`currentUser`),n=E(`loggedInPlayerIndex`),r=E(`NAMES`),i=E(`players`);if(t){if(n!==-1&&r[n])e.authBtn&&(e.authBtn.innerText=`Logout (${r[n]})`);else{let n=t.email?t.email.split(`@`)[0]:`User`;e.authBtn&&(e.authBtn.innerText=`Logout (${n})`)}e.adminNav&&(e.adminNav.style.display=K()?`flex`:`none`);let a=n===-1?null:i[n];e.avatarBtn&&(a?(e.avatarBtn.classList.add(`logged-in`),e.avatarBtn.style.setProperty(`--avatar-color`,`var(--${a.id})`)):(e.avatarBtn.classList.remove(`logged-in`),e.avatarBtn.style.removeProperty(`--avatar-color`)))}else{e.authBtn&&(e.authBtn.innerText=`Login`),e.adminNav&&(e.adminNav.style.display=`none`);let t=document.getElementById(`adminSection`);t&&t.classList.add(`hidden`),e.actionButtons&&(e.actionButtons.style.display=`none`),e.avatarBtn&&(e.avatarBtn.classList.remove(`logged-in`),e.avatarBtn.style.removeProperty(`--avatar-color`))}}function ve(){let e=A();e.accountModal&&(e.accountModal.style.display=`flex`),document.body.style.overflow=`hidden`}function ye(){let e=A();e.accountModal&&(e.accountModal.style.display=`none`),document.body.style.overflow=`auto`}function be(){let e=A(),t=E(`players`);!e.playerTogglesContainer||!t||t.length===0||(e.playerTogglesContainer.innerHTML=t.slice(0,4).map((e,t)=>`
            ...
            </label>`).join(``))}function xe(){let e=A();e.loginModal&&(e.loginModal.style.display=`flex`),e.loginError&&(e.loginError.style.display=`none`),Te(),document.body.style.overflow=`hidden`}function Se(){let e=A();e.loginModal&&(e.loginModal.style.display=`none`),Te(),document.body.style.overflow=`auto`}function Ce(e){let t=A();t.loginError&&(t.loginError.innerText=e,t.loginError.style.color=`var(--danger)`,t.loginError.style.fontSize=`0.95rem`,t.loginError.style.fontWeight=`600`,t.loginError.style.display=`block`)}function we(e){Ce(e);let t=A();t.loginEmailInput&&(t.loginEmailInput.style.borderColor=`var(--danger)`,t.loginEmailInput.style.boxShadow=`0 0 0 2px color-mix(in srgb, var(--danger) 25%, transparent)`)}function Te(){let e=A();e.loginEmailInput&&(e.loginEmailInput.style.borderColor=`#ccc`,e.loginEmailInput.style.boxShadow=``)}function Ee(){let e=A();e.updatePasswordModal&&(e.updatePasswordModal.style.display=`block`),e.updatePasswordError&&(e.updatePasswordError.style.display=`none`),document.body.style.overflow=`hidden`;let t=E(`currentUser`);e.updatePasswordUsername&&t&&(e.updatePasswordUsername.value=t.email||``)}function De(){let e=A();e.updatePasswordModal&&(e.updatePasswordModal.style.display=`none`),document.body.style.overflow=`auto`}function Oe(e){let t=A();t.updatePasswordError&&(t.updatePasswordError.innerText=e,t.updatePasswordError.style.color=`var(--danger)`,t.updatePasswordError.style.display=`block`)}function ke(){let e=A();e.loginError&&(e.loginError.innerText=`Password reset email sent. Please check your inbox.`,e.loginError.style.color=`#4CAF50`,e.loginError.style.fontSize=`0.95rem`,e.loginError.style.fontWeight=`600`,e.loginError.style.display=`block`,setTimeout(()=>{e.loginError&&(e.loginError.style.display=`none`,e.loginError.innerText=``,e.loginError.style.color=`var(--danger)`)},5e3))}function Ae(){let e=A();e.newPasswordInput&&(e.newPasswordInput.value=``),e.confirmPasswordInput&&(e.confirmPasswordInput.value=``)}var je=null;function Me(){_e(),H(),G(),di()}function Ne(){be()}function Pe(){xe(),je=e=>{e.key===`Escape`&&Fe()},document.addEventListener(`keydown`,je)}function Fe(){Se(),je&&document.removeEventListener(`keydown`,je)}function Ie(){ve()}function Le(){ye()}async function Re(){let e=document.getElementById(`login-email`).value,t=document.getElementById(`login-password`).value,{error:n}=await c({email:e,password:t});n&&Ce(n.message)}async function ze(){let e=document.getElementById(`login-email`).value;if(!e){we(`Please enter your email address first.`);return}Te();let{error:t}=await l(e);t?Ce(t.message||`Failed to send reset email. Please try again.`):ke()}function Be(){Ee()}function Ve(){De()}async function He(){let e=document.getElementById(`new-password`).value,t=document.getElementById(`confirm-password`).value;if(!e){Oe(`Please enter a new password.`);return}if(e!==t){Oe(`Passwords do not match.`);return}let{error:n}=await u({password:e});n?Oe(n.message):(alert(`Password updated successfully!`),Ve(),Ae())}async function Ue(){await k(`Log Out`,`Log out now?`)&&await d()}var j={duel:`<svg viewBox="0 -960 960 960" width="20" height="20" fill="currentColor"><path d="M769-88 645-212l-88 88-43-43q-17-17-17-42t17-42l199-199q17-17 42-17t42 17l43 43-88 88 123 124q9 9 9 21t-9 21l-64 65q-9 9-21 9t-21-9Zm111-636L427-271l19 20q17 17 17 42t-17 42l-43 43-88-88L191-88q-9 9-21 9t-21-9l-65-65q-9-9-9-21t9-21l124-124-88-88 43-43q17-17 42-17t42 17l20 19 453-453h160v160ZM278-526 80-724v-160h160l198 198-160 160Z"/></svg>`,teams:`<svg viewBox="0 -960 960 960" width="20" height="20" fill="currentColor"><path d="M38-160v-94q0-35 18-63.5t50-42.5q73-32 131.5-46T358-420q62 0 120 14t131 46q32 14 50.5 42.5T678-254v94H38Zm700 0v-94q0-63-32-103.5T622-423q69 8 130 23.5t99 35.5q33 19 52 47t19 63v94H738ZM250-523q-42-42-42-108t42-108q42-42 108-42t108 42q42 42 42 108t-42 108q-42 42-108 42t-108-42Zm426 0q-42 42-108 42-11 0-24.5-1.5T519-488q24-25 36.5-61.5T568-631q0-45-12.5-79.5T519-774q11-3 24.5-5t24.5-2q66 0 108 42t42 108q0 66-42 108Z"/></svg>`,ffa:`<svg viewBox="0 -960 960 960" width="20" height="20" fill="currentColor"><path d="M420-340h120l-60-120-60 120Zm-79.91-120q28.91 0 49.41-20.59 20.5-20.59 20.5-49.5t-20.59-49.41q-20.59-20.5-49.5-20.5t-49.41 20.59q-20.5 20.59-20.5 49.5t20.59 49.41q20.59 20.5 49.5 20.5Zm280 0q28.91 0 49.41-20.59 20.5-20.59 20.5-49.5t-20.59-49.41q-20.59-20.5-49.5-20.5t-49.41 20.59q-20.5 20.59-20.5 49.5t20.59 49.41q20.59 20.5 49.5 20.5ZM240-80v-170q-36-16-65.5-43T124-355.5Q103-391 91.5-433T80-520q0-158 112-259t288-101q176 0 288 101t112 259q0 45-11.5 87T836-355.5Q815-320 785.5-293T720-250v170H600v-120h-60v120H420v-120h-60v120H240Z"/></svg>`,koth:`<svg viewBox="0 -960 960 960" width="20" height="20" fill="currentColor"><path d="M510-80v-329q0-23 5.5-44.5T537-492q44-46 103-72.5T767-591q63 0 108 45t45 108v205q0 63-45 108T767-80H510Zm-317 0q-63 0-108-45T40-233v-205q0-63 45-108t108-45q69 0 129.5 27.5T427-488q14 16 18.5 36.5T450-409v329H193Zm287-444q-29-28-58.5-49.5T359-612q17-22 40-37t51-21v-66H327v-60h123v-124h60v124h123v60H510v66q28 5 51 20t40 37q-33 17-62.5 39T480-524Z"/></svg>`},We=`<svg class="setup-hint-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="16" x2="12" y2="12"></line><line x1="12" y1="8" x2="12.01" y2="8"></line></svg>`,Ge=`<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="5" y1="12" x2="19" y2="12"></line><polyline points="13 6 19 12 13 18"></polyline></svg>`,Ke={duel:`1v1`,"2v2":`2v2`,"3v3":`3v3`,ffa:`FFA`,koth:`KotH`},qe={duel:`Standard health pools (50 HP), each player has their CP.`,"2v2":`Teams share a single health pool (50 HP for 2v2), each player has their CP.`,"3v3":`Teams share a single health pool (60 HP for 3v3), each player has their CP.`,ffa:`Standard health pools (50 HP), each player has their CP, targeting using dice (1/2 left, 3/4 across, 5/6 right). With 3 players, 3/4 is player's choice.`,koth:`Free For All variant when you may freely choose any opponent, but if you attack the highest health player, you immediately draw a bonus card.`},Je=null,M=()=>(Je||={gameTypeStep:document.getElementById(`setup-step-gametype`),gameTypeGrid:document.getElementById(`game-type-grid`),gameTypeHint:document.getElementById(`game-type-hint`),inviteeZone:document.getElementById(`invitee-zone`),teamsStep:document.getElementById(`setup-step-teams`),teamsGrid:document.getElementById(`teams-grid`),randomizeTeamsBtn:document.getElementById(`randomize-teams-btn`),teamSwapQuestion:document.getElementById(`team-swap-question`),teamSwapScrim:document.getElementById(`team-swap-scrim`),rollModeStep:document.getElementById(`setup-step-rollmode`),rollModeGrid:document.getElementById(`roll-mode-grid`),draftCountSection:document.getElementById(`draft-count-section`),draftCountGrid:document.getElementById(`draft-count-grid`),rollStep:document.getElementById(`setup-step-roll`),rollFinalBtn:document.getElementById(`roll-final-btn`)},Je);function N(){let e=M();if(!e.inviteeZone)return;let t=E(`invitees`).map(e=>`
            ...
            </button>`:``;e.inviteeZone.innerHTML=t+n}function Ye(e=ct()){let t=M();t.gameTypeStep&&t.gameTypeStep.classList.toggle(`step-locked`,e<2)}function Xe(){let e=M();if(!e.gameTypeGrid)return;let t=pt(ct()),n=E(`selectedGameType`),r=[{type:`duel`,label:`1v1 Duel`,enabled:t.duel,icon:j.duel},{type:t.teamsValue,label:`Teams ${t.teamsValue}`,enabled:t.teams,icon:j.teams},{type:`ffa`,label:`Free For All`,enabled:t.ffa,icon:j.ffa},{type:`koth`,label:`King of the Hill`,enabled:t.koth,icon:j.koth}];e.gameTypeGrid.innerHTML=r.map(e=>`
            <button type="button"
            ...
            </button>`).join(``),Ze(n)}function Ze(e=E(`selectedGameType`)){let t=M();if(!t.gameTypeHint)return;let n=e&&qe[e];if(!n){t.gameTypeHint.style.display=`none`,t.gameTypeHint.innerHTML=``;return}t.gameTypeHint.style.display=`flex`,t.gameTypeHint.innerHTML=`${We}<span>${n}</span>`}function Qe(e,t,n,r){let i=!!r&&r.team!==e,a=t.map(t=>{let a=n.get(t);if(!a)return``;if(i)return`
                ...
                </button>`;let o=!!r&&r.id===a.id,s=e===`B`,c=`<span class="team-player-arrow" aria-hidden="true">${Ge}</span>`,l=`<span class="team-player-dot"></span><span class="team-player-name">${a.name}</span>`;return`
        ...
        </div>`}function P(){let e=M();if(!e.teamsStep)return;if(!st(E(`selectedGameType`))){e.teamsStep.style.display=`none`,e.teamSwapScrim.style.display=`none`;return}e.teamsStep.style.display=`block`;let t=E(`teamAssignments`);if(!t){e.teamsGrid.innerHTML=``;return}let n=E(`teamSwapSource`),r=new Map(ut().map(e=>[e.id,e]));e.teamsGrid.innerHTML=Qe(`A`,t.teamA,r,n)+Qe(`B`,t.teamB,r,n),e.randomizeTeamsBtn.style.display=n?`none`:`block`,e.teamSwapQuestion.style.display=n?`block`:`none`,e.teamSwapQuestion.classList.toggle(`swap-active`,!!n),e.teamSwapScrim.style.display=n?`block`:`none`}var $e=[2,3,4,5];function et(){let e=M();e.rollModeStep&&e.rollModeStep.classList.toggle(`step-locked`,!E(`selectedGameType`))}function F(){let e=M();if(!e.rollModeGrid)return;let t=E(`rollModeChosen`),n=E(`draftModeEnabled`),r=E(`draftCountChosen`),i=E(`draftCount`),a=!E(`selectedGameType`);e.rollModeGrid.innerHTML=`
        ...
        </button>`,e.draftCountSection.style.display=t&&n?`block`:`none`,e.draftCountGrid.innerHTML=$e.map(e=>`
        <button type="button" class="draft-count-btn${r&&i===e?` active`:``}" data-action="select-draft-count" data-count="${e}">${e}</button>`).join(``)}function I(){let e=M();if(!e.rollStep||!e.rollFinalBtn)return;let t=E(`selectedGameType`),n=E(`rollModeChosen`),r=E(`draftModeEnabled`),i=E(`draftCountChosen`),a=!r||i;if(e.rollStep.classList.toggle(`step-locked`,!t||!n||!a),!t){e.rollFinalBtn.disabled=!0,e.rollFinalBtn.innerHTML=`<span>ROLL &middot; Complete the steps above</span>`;return}if(!n||!a){e.rollFinalBtn.disabled=!0,e.rollFinalBtn.innerHTML=`<span>ROLL &middot; Finish selecting your options above</span>`;return}e.rollFinalBtn.disabled=!1;let o=st(t)?j.teams:j[t],s=Ke[t],c=ct();e.rollFinalBtn.innerHTML=`${o}<span>ROLL &middot; ${s} &middot; ${c} PLAYER${c===1?``:`S`}</span>`}function tt(){let e=document.getElementById(`randomizer-setup`);e&&(e.style.display=`none`);let t=document.getElementById(`team-swap-scrim`);t&&(t.style.display=`none`)}function nt(){let e=document.getElementById(`randomizer-setup`);e&&(e.style.display=`block`);let t=document.getElementById(`results-title`);t&&(t.style.display=`none`)}var rt={draft:`ROLL &middot; DRAFT`,confirmation:`ROLL &middot; CONFIRMATION`};function it(e){let t=document.getElementById(`results-title`);t&&(t.innerHTML=rt[e]||``,t.style.display=`block`)}function at(){N(),Ye(),Xe(),P(),et(),F(),I()}var ot=6;function st(e){return e===`2v2`||e===`3v3`}function ct(){return document.querySelectorAll(`#player-toggle-zone-top input:checked, #invitee-zone input:checked`).length}function lt(){return document.querySelectorAll(`#player-toggle-zone-top input:checked`).length+E(`invitees`).length<ot}function ut(){let e=E(`players`).slice(0,4),t=E(`invitees`),n=[];return e.forEach((e,t)=>{document.getElementById(`use${t}`)?.checked&&n.push({id:e.id,name:e.name,colorVar:e.id})}),t.forEach(e=>{document.querySelector(`#invitee-zone input[data-invitee-id="${e.id}"]`)?.checked&&n.push({id:e.id,name:e.name,colorVar:`p5`})}),n}function dt(){let e=E(`players`).slice(0,4),t=E(`invitees`),n=[];e.forEach((e,t)=>{document.getElementById(`use${t}`)?.checked&&n.push({pIdx:t,name:e.name,colorVar:e.id,isInvitee:!1})});let r=0;return t.forEach(e=>{document.querySelector(`#invitee-zone input[data-invitee-id="${e.id}"]`)?.checked&&(n.push({pIdx:4+r,name:e.name,colorVar:`p5`,isInvitee:!0}),r++)}),n}function ft(){let e=ut(),t=dt(),n={};return e.forEach((e,r)=>{n[e.id]=`p${t[r].pIdx+1}`}),n}function pt(e){return{duel:e===2,teams:e===4||e===6,ffa:e>=3&&e<=6,koth:e>=3&&e<=6,teamsValue:e===6?`3v3`:`2v2`}}function mt(){if(!lt())return;let e=E(`invitees`);D(`invitees`,[...e,{id:`invitee-${Date.now()}`,name:`Invitee ${e.length+1}`}]),N(),yt()}function ht(e){D(`invitees`,E(`invitees`).filter(t=>t.id!==e).map((e,t)=>({...e,name:`Invitee ${t+1}`}))),N(),yt()}function gt(){D(`invitees`,[]),N(),yt()}function _t(){D(`invitees`,[]),D(`selectedGameType`,null),D(`teamAssignments`,null),D(`teamSwapSource`,null),D(`rollModeChosen`,!1),D(`draftCountChosen`,!1),N(),yt()}function vt(e){if(!e)return;let t=pt(ct());(e===`duel`&&t.duel||e===t.teamsValue&&t.teams||e===`ffa`&&t.ffa||e===`koth`&&t.koth)&&(D(`selectedGameType`,e),D(`teamSwapSource`,null),st(e)?St():D(`teamAssignments`,null),Xe(),P(),et(),F(),I())}function yt(){let e=ct(),t=pt(e),n=E(`selectedGameType`),r=n===`duel`&&t.duel||n===t.teamsValue&&t.teams||n===`ffa`&&t.ffa||n===`koth`&&t.koth;n&&!r&&D(`selectedGameType`,null);let i=E(`selectedGameType`);D(`teamSwapSource`,null),st(i)?St():D(`teamAssignments`,null),Ye(e),Xe(),P(),et(),F(),I()}function bt(e){let t=e===`draft`;D(`rollModeChosen`,!0),D(`draftModeEnabled`,t),localStorage.setItem(`draftModeEnabled`,t),F(),I()}function xt(e){D(`draftCount`,e),D(`draftCountChosen`,!0),localStorage.setItem(`draftCount`,e),F(),I()}function St(){let e=[...ut()].sort(()=>Math.random()-.5),t=e.length/2;D(`teamAssignments`,{teamA:e.slice(0,t).map(e=>e.id),teamB:e.slice(t).map(e=>e.id)}),D(`teamSwapSource`,null),P()}function Ct(e,t){D(`teamSwapSource`,{id:e,team:t}),P()}function wt(){D(`teamSwapSource`,null),P()}function Tt(e){let t=E(`teamSwapSource`),n=E(`teamAssignments`);if(!t||!n)return;let{teamA:r,teamB:i}=n,a=t.team===`A`?r:i,o=t.team===`A`?i:r,s=a.indexOf(t.id),c=o.indexOf(e);s===-1||c===-1||(a[s]=e,o[c]=t.id,D(`teamAssignments`,{teamA:r,teamB:i}),D(`teamSwapSource`,null),P())}var Et=null,L=()=>(Et||={sortSection:document.getElementById(`sort-section`),sortToggleBtn:document.getElementById(`sort-panel-toggle`),filterSection:document.getElementById(`filter-section`),filterToggleBtn:document.getElementById(`filter-panel-toggle`),sortFilterDrawer:document.getElementById(`sort-filter-drawer`),drawerTitle:document.getElementById(`drawer-title-text`),drawerFooter:document.getElementById(`drawer-footer-content`),drawerBody:document.getElementById(`drawer-body-content`),leftFilterDrawer:document.getElementById(`filter-drawer-left`),leftPlayersContainer:document.getElementById(`filter-options-players`),leftGroupsContainer:document.getElementById(`filter-options-groups`),leftHeroCountLabel:document.getElementById(`filter-drawer-hero-count`),leftTitleDataHistory:document.getElementById(`title-data-history`),leftTitlePlayers:document.getElementById(`title-players`),leftTitleComplexity:document.getElementById(`title-complexity`),leftTitleGroups:document.getElementById(`title-groups`),filterActiveBadge:document.getElementById(`filter-active-badge`),gamesFilterActiveBadge:document.getElementById(`games-filter-active-badge`),sortTriggerBtn:document.getElementById(`btn-trigger-sort`),sortDropdownMenu:document.getElementById(`sort-dropdown-menu`),activeFiltersContainer:document.getElementById(`active-filters-container`),heroContainer:document.getElementById(`heroContainer`),countStatsLabel:document.getElementById(`count-stats`),heroSearchInput:document.getElementById(`hero-search`),dbShowOwnedCheckbox:document.getElementById(`db-show-owned`),dbShowNotOwnedCheckbox:document.getElementById(`db-show-not-owned`)},Et);function Dt(){let e=L();e.sortFilterDrawer&&(e.drawerTitle&&(e.drawerTitle.innerText=`Filter History`),e.drawerFooter&&(e.drawerFooter.style.display=`flex`),Ut(),e.sortFilterDrawer.classList.add(`open`),document.body.style.overflow=`hidden`)}function Ot(){let e=E(`stagedOwnershipFilter`);Object.entries({owned:`pill-show-owned`,unowned:`pill-show-not-owned`,all:`pill-show-all`}).forEach(([t,n])=>{document.getElementById(n)?.classList.toggle(`active`,t===e)}),Ft()}function kt(){Mt(),Ot(),Bt(),Vt();let e=L();e.leftFilterDrawer&&(e.leftFilterDrawer.classList.add(`open`),document.body.style.overflow=`hidden`)}function At(e=null,t=!1){if(e&&e.target!==e.currentTarget&&!t)return;let n=L();n.leftFilterDrawer&&(n.leftFilterDrawer.classList.remove(`open`),document.body.style.overflow=`auto`)}function jt(e=null,t=!1){if(e&&e.target!==e.currentTarget&&!t)return;let n=L();n.sortFilterDrawer&&(n.sortFilterDrawer.classList.remove(`open`),document.body.style.overflow=`auto`)}function Mt(){let e=L(),t=E(`players`),n=E(`groups`),r=E(`stagedFilterPlayers`),i=E(`stagedFilterGroups`),a=E(`stagedFilterDataHistories`),o=E(`stagedFilterComplexities`);if(e.leftPlayersContainer&&t){let n=t.filter(e=>e.name&&!e.name.toLowerCase().includes(`invitee`)).slice().sort((e,t)=>e.name.localeCompare(t.name));e.leftPlayersContainer.innerHTML=n.map(e=>{let t=r.has(e.id)?`checked`:``;return`
            ...
            `}).join(``)}document.querySelectorAll(`#filter-drawer-left input[data-type="data-history"]`).forEach(e=>{e.checked=a.has(e.value)}),document.querySelectorAll(`#filter-drawer-left input[data-type="complexity"]`).forEach(e=>{e.checked=o.has(Number(e.value))})}function Nt(){let e=L();if(!e.filterActiveBadge)return;let t=E(`activeFilterDataHistories`),n=E(`activeFilterPlayers`),r=E(`activeFilterComplexities`),i=E(`activeFilterGroups`),a=E(`activeOwnershipFilter`),o=0;t&&(o+=t.size),n&&(o+=n.size),r&&(o+=r.size),i&&(o+=i.size),a&&a!==`all`&&o++,o>0?(e.filterActiveBadge.innerText=o,e.filterActiveBadge.style.display=`inline-block`):e.filterActiveBadge.style.display=`none`}function Pt(){let e=L();if(!e.gamesFilterActiveBadge)return;let t=E(`selectedGamePlayerIndex`),n=E(`gamesWinnerOnly`),r=E(`gamesUseHistorical`),i=0;t!==null&&i++,n&&i++,r||i++,i>0?(e.gamesFilterActiveBadge.innerText=i,e.gamesFilterActiveBadge.style.display=`inline-block`):e.gamesFilterActiveBadge.style.display=`none`}function Ft(){document.querySelectorAll(`.ownership-segmented-control, .segmented-control`).forEach(e=>{let t=e.querySelector(`.segmented-pill.active`),n=e.querySelector(`.segmented-highlight`);n||(n=document.createElement(`div`),n.className=`segmented-highlight`,e.insertBefore(n,e.firstChild)),t&&(n.style.width=`${t.offsetWidth}px`,n.style.transform=`translateX(${t.offsetLeft}px)`,n.style.height=`${t.offsetHeight}px`)})}function It(){let e=L();if(!e.sortTriggerBtn)return;let t=E(`currentSort`),n=E(`sortAsc`),r=E(`currentSortPlayerIndex`),i=E(`NAMES`),a=`Hero (A-Z)`;if(t===`name`)a=n?`Hero (A-Z)`:`Hero (Z-A)`;else if(t===`complexity`)a=n?`Complexity (1-6)`:`Complexity (6-1)`;else if(t.startsWith(`w`)){let e=i[r]||`Player ${r+1}`;a=n?`${e} % (Low to High)`:`${e} % (High to Low)`}else if(t.startsWith(`d`)){let e=i[r]||`Player ${r+1}`;a=n?`${e} Played (Oldest)`:`${e} Played (Newest)`}else t===`group`&&(a=n?`Group (A-Z)`:`Group (Z-A)`);e.sortTriggerBtn.innerHTML=`<span class="action-icon">⇅</span> <strong style="font-weight: 700;">SORT:</strong> <span style="font-weight: 400; text-transform: none; margin-left: 2px;">${a}</span>`}function Lt(){let e=L();if(!e.sortDropdownMenu)return;let t=E(`currentSort`),n=E(`sortAsc`),r=E(`activePlayerIndices`),i=E(`NAMES`),a=`
    ...
    `}function Ut(){let e=L();if(!e.drawerBody)return;let t=E(`currentDrawerMode`),n=E(`stagedSort`);E(`stagedSortPlayerIndex`);let r=E(`stagedPlayerIndices`),i=E(`stagedUseHistorical`),a=E(`NAMES`);E(`activePlayerIndices`);let o=E(`stagedBanSearchQuery`);if(e.drawerBody.style.overflowY=`auto`,t===`sort-filter`){e.drawerBody.innerHTML=`
            <div class="panel-row-new">
        ...
        `,pn())}function Wt(){let e=document.getElementById(`drawer-sort-direction-text`),t=document.getElementById(`drawer-sort-direction-arrow`),n=E(`stagedSortAsc`);e&&t&&(e.innerText=n?`Ascending`:`Descending`,t.innerText=n?`▲`:`▼`)}function Gt(){let e=document.getElementById(`drawer-player-sort-sub-section`),t=document.getElementById(`drawer-player-sort-pills`);if(!e||!t)return;let n=E(`stagedSort`),r=E(`stagedSortPlayerIndex`),i=E(`activePlayerIndices`),a=E(`NAMES`),o=n.startsWith(`w`)||n.startsWith(`d`);e.style.display=o?`block`:`none`,o&&(t.innerHTML=a.slice(0,4).map((e,t)=>`
    ...
    `}function Xt(){let e=L();if(!e.activeFiltersContainer)return;let t=e.heroSearchInput,n=t?t.value.trim():``,r=E(`activeOwnershipFilter`),i=E(`activeFilterDataHistories`),a=E(`activeFilterPlayers`),o=E(`activeFilterComplexities`),s=E(`activeFilterGroups`),c=E(`players`),l=E(`groups`),u=``;n&&(u+=`
        ...
        `),i&&Array.from(i).sort((e,t)=>{let n=[`Normal only`,`Historical only`];return n.indexOf(e)-n.indexOf(t)}).forEach(e=>{u+=`
            ...
            `}),e.activeFiltersContainer.innerHTML=u,e.activeFiltersContainer.classList.toggle(`has-chips`,!!u.trim())}function Zt(){let e=L();if(!e.heroContainer)return;Li();let t=e.heroSearchInput?.value.toLowerCase()||``,n=e.dbShowOwnedCheckbox?.checked??!0,r=e.dbShowNotOwnedCheckbox?.checked??!1,i=E(`NAMES`),a=E(`characters`),o=E(`activeFilterComplexities`),s=E(`activeFilterGroups`),c=E(`activeFilterDataHistories`),l=E(`activeFilterPlayers`),u=E(`games`),d=E(`activePlayerIndices`),f=E(`currentSort`),p=E(`sortAsc`);E(`currentSortPlayerIndex`);let m=(e,t)=>{if(!t)return!0;let n=t.trim().toLowerCase();return(e.name||``).toLowerCase().includes(n)||(e.group||``).toLowerCase().includes(n)},h=[];t&&i.forEach((e,n)=>{e&&e.toLowerCase().includes(t)&&h.push(n)});let g=[,,,,].fill(0);a.filter(X).forEach(e=>{for(let t=0;t<4;t++)g[t]+=Q(e,t)});let _=a.map((e,t)=>({...e,originalIndex:t})).filter(e=>{let i=!0;o.size>0&&(i=o.has(Number(e.complexity)));let a=!0;s.size>0&&(a=s.has(e.group_id));let d=!0,f=c.has(`Normal only`),p=c.has(`Historical only`);f&&!p?d=u.filter(t=>t.game_players.some(t=>t.hero_id===e.id)).some(e=>!e.is_historical):p&&!f&&(d=u.filter(t=>t.game_players.some(t=>t.hero_id===e.id)).some(e=>e.is_historical));let h=!0;l.size>0&&(h=u.filter(t=>t.game_players.some(t=>t.hero_id===e.id)).some(t=>t.game_players.some(t=>t.hero_id===e.id&&l.has(t.player_id))));let g=X(e)&&n||!X(e)&&r;return m(e,t)&&i&&a&&d&&h&&g});e.countStatsLabel&&(e.countStatsLabel.innerText=`Showing ${_.length} of ${a.length} heroes`),_.sort((e,t)=>{let n,r;if(f.startsWith(`w`)){let i=parseInt(f[1]);n=Q(e,i),r=Q(t,i)}else if(f.startsWith(`d`)){let i=parseInt(f[1]);n=e.lastPlayed&&e.lastPlayed[i]||``,r=t.lastPlayed&&t.lastPlayed[i]||``,(n===`Never`||n===`Unknown`)&&(n=``),(r===`Never`||r===`Unknown`)&&(r=``)}else if(f===`group`){if(n=(e.group||``).toLowerCase(),r=(t.group||``).toLowerCase(),n===r){let n=(e.name||``).toLowerCase(),r=(t.name||``).toLowerCase();return p?n.localeCompare(r):r.localeCompare(n)}}else if(f===`complexity`){if(n=Number(e.complexity)||0,r=Number(t.complexity)||0,n===r){let n=(e.name||``).toLowerCase(),r=(t.name||``).toLowerCase();return n.localeCompare(r)}}else n=(e[f]||``).toLowerCase(),r=(t[f]||``).toLowerCase();if(n===r)return 0;let i=n<r?-1:1;return p?i:-i}),e.heroContainer.innerHTML=_.map(e=>{let t=d;l.size>0?t=Array.from(l).map(e=>parseInt(e.substring(1))-1).filter(e=>e>=0&&e<4):h.length>0&&(t=d.filter(e=>h.includes(e)));let n=t.map(t=>{let n=Q(e,t),r=X(e)&&g[t]>0?(n/g[t]*100).toFixed(2):`0.00`,i=e.playCount&&e.playCount[t]||0,a=e.lastPlayed&&e.lastPlayed[t]||`Never`,o=e.winCount&&e.winCount[t]||0,s=i>0?(o/i*100).toFixed(1):`0.0`;return{p:t,percentage:parseFloat(r),percentageStr:r,playCount:i,lastPlayed:a,winCount:o,winRate:s}});n.sort((e,t)=>{let n=(i[e.p]||``).toLowerCase(),r=(i[t.p]||``).toLowerCase();return n.localeCompare(r)});let r=n.map(e=>{let t=Ki(e.lastPlayed);return`
                ...
                </div>`}).join(``),a=n.map(e=>{let t=Gi(e.lastPlayed),n=Ki(e.lastPlayed),r=t?`<span class="expanded-player-relative">${t} ${n}</span>`:``;return`
    ...
    `}function en(e){let t=R();if(!t.heroSelectModal)return;t.heroSelectModal.style.display=`flex`,document.body.style.overflow=`hidden`;let n=q(e);t.heroSelectModalTitle&&n?.name&&(t.heroSelectModalTitle.innerText=`Select Hero for ${n.name}`),t.heroSelectSearch&&(t.heroSelectSearch.value=``),D(`modalSortMode`,`name`),nn(`name`),rn(),setTimeout(Ft,50)}function tn(){let e=R();e.heroSelectModal&&(e.heroSelectModal.style.display=`none`),document.body.style.overflow=``}function nn(e){let t=R();t.modalSortName&&t.modalSortName.classList.toggle(`active`,e===`name`),t.modalSortWeight&&t.modalSortWeight.classList.toggle(`active`,e===`weight`),Ft()}function rn(){let e=R();if(!e.heroSelectOptionsContainer)return;let t=E(`activeSelectPlayerIdx`),n=E(`modalSortMode`),r=e.heroSelectSearch,i=r?r.value.toLowerCase().trim():``,a=E(`characters`),o=E(`bannedHeroIds`);if(t===null)return;let s=a.filter(e=>X(e)&&!o.has(e.id));i&&(s=s.filter(e=>e.name.toLowerCase().includes(i)||e.group&&e.group.toLowerCase().includes(i))),n===`name`?s.sort((e,t)=>e.name.localeCompare(t.name)):n===`weight`&&s.sort((e,n)=>{let r=Q(e,t),i=Q(n,t);return r===i?e.name.localeCompare(n.name):i-r});let c=document.getElementById(`select-${t}`)?.value;if(s.length===0){e.heroSelectOptionsContainer.innerHTML=`<p style="opacity: 0.6; font-style: italic; grid-column: 1 / -1; text-align: center; padding: 20px;">No available heroes found.</p>`;return}e.heroSelectOptionsContainer.innerHTML=s.map(e=>{let n=c===e.name,r=Hi(e,t);return`
    ...
    `;let s=document.getElementById(`stats-row-${e}`);if(s)if(e<4){let n=`Prob: <b>${Hi(t,e)}</b>`;s.innerHTML=`
                <span>Plays: <b>${t.playCount[e]||0}</b></span>
                <span class="stats-divider">|</span>
                <span>Last: <b>${t.lastPlayed[e]||`Never`}</b></span>
                <span class="stats-divider">|</span>
    ...
    `}function cn(e,t){let n=document.getElementById(`draft-card-list`);if(!n)return;let r=``;for(let n=0;n<t;n++)r+=`
        ...
        `}).join(``)}var mn=`<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"></rect><path d="M7 11V7a5 5 0 0 1 10 0v4"></path></svg>`;function hn(){let e=E(`characters`),t=E(`bannedHeroIds`),n=dt();if(console.log(`[randomizer] pickCharacters participants:`,n),console.log(`[randomizer] draftModeEnabled:`,E(`draftModeEnabled`)),n.length===0)return alert(`Select players!`);D(`activeRollParticipants`,n);let r=n.map(e=>e.pIdx),i=[...r].sort(()=>Math.random()-.5),a=document.getElementById(`results`);a&&(a.innerHTML=``);let o=e.filter(e=>X(e)&&!t.has(e.id)).map(e=>structuredClone(e));if(console.log(`[randomizer] Owned/non-banned heroes pool count:`,o.length),o.length<r.length)return alert(`Not enough available (owned & non-banned) heroes (${o.length}) in your collection for ${r.length} players!`);if(tt(),E(`draftModeEnabled`)){let e=document.getElementById(`action-buttons`);e&&(e.style.display=`none`),D(`activeDraftOrder`,i),D(`activeDraftStep`,0),D(`selectedDraftHeroes`,{}),D(`activeDraftCandidates`,{}),Zr(`roll`),it(`draft`),a&&a.scrollIntoView({behavior:`smooth`,block:`start`}),An();return}let s={};i.forEach(e=>{let t=null;if(e>=4){let e=Math.floor(Math.random()*o.length);t=o[e],o.splice(e,1)}else{let n=o.filter(t=>t.weights[e]>0);if(n.length===0){if(o.length>0){let e=Math.floor(Math.random()*o.length);t=o[e],o.splice(e,1)}}else{let r=n.reduce((t,n)=>t+Q(n,e),0),i=Math.random()*r;for(let r of n){let n=Q(r,e);if(i<n){t=r,o.splice(o.findIndex(e=>e.name===r.name),1);break}i-=n}}}s[e]=t});let c=document.getElementById(`action-buttons`);c&&(c.style.display=`none`),D(`isRollActive`,!1);let l=[...r].sort((e,t)=>e-t);l.forEach(e=>{$t(e)}),Zr(`roll`),it(`confirmation`),a&&a.scrollIntoView({behavior:`smooth`,block:`start`});let u=e.filter(e=>X(e)&&!t.has(e.id));l.forEach(e=>{xn(e,u)});let d=0;function f(){if(d>=i.length){Cn(),Ri()?c&&(c.style.display=`flex`):(nt(),gt()),D(`isRollActive`,!0);return}let e=i[d],t=s[e],n=500+Math.random()*500;setTimeout(()=>{Sn(e,t),d++,setTimeout(f,400)},n)}f()}function z(){Cn()}function gn(e){D(`activeSelectPlayerIdx`,e),en(e)}function _n(){tn(),D(`activeSelectPlayerIdx`,null)}function vn(e){D(`modalSortMode`,e),nn(e),yn()}function yn(){rn()}function bn(e){let t=E(`activeSelectPlayerIdx`);if(t===null)return;let n=E(`characters`).find(t=>t.name===e);n&&(E(`draftModeEnabled`)&&(E(`selectedDraftHeroes`)[t]=n),an(t,n),Cn(),_n())}function xn(e,t){if(t.length===0)return;let n=document.getElementById(`bg-img-${e}`),r=document.getElementById(`hero-name-title-${e}`);me(`scrambleIntervals`,e,setInterval(()=>{let e=t[Math.floor(Math.random()*t.length)];if(n&&(n.src=J(e.slug)),r){let e=``;for(let t=0;t<8;t++)e+=`ABCDEFGHIJKLMNOPQRSTUVWXYZ@#$%&*`[Math.floor(Math.random()*32)];r.innerText=e}},70))}function Sn(e,t){let n=E(`scrambleIntervals`);n[e]&&(clearInterval(n[e]),me(`scrambleIntervals`,e,void 0)),t&&an(e,t)}function Cn(){let e=document.querySelectorAll(`.char-select`),t=Array.from(e).map(e=>e.value),n=t.reduce((e,t)=>(e[t]=(e[t]||0)+1,e),{}),r=Object.values(n).some(e=>e>1),i=E(`characters`),a=t.filter(e=>{let t=i.find(t=>t.name===e);return t&&!X(t)}),o=a.length>0,s=document.getElementById(`confirmBtn`),c=document.getElementById(`error-msg`);!s||!c||(s.classList.remove(`disabled`,`warning`),s.disabled=!1,s.innerHTML=`${mn} LOCK IN SESSION`,c.style.display=`none`,r?(s.classList.add(`disabled`),s.disabled=!0,c.style.display=`block`,c.innerText=`⚠ Duplicate hero selected! Each player must have a unique character.`):o&&(s.classList.add(`warning`),s.innerHTML=`⚠️ LOCK IN SESSION`,c.style.display=`block`,c.innerText=`⚠️ You have selected unowned heroes: ${a.join(`, `)}.`),e.forEach(e=>{let t=e.closest(`.player-row`);t&&t.classList.toggle(`error`,n[e.value]>1)}))}async function wn(){let e=document.getElementById(`confirmBtn`),t=e?e.innerHTML:`Lock In`;e&&(e.disabled=!0,e.innerText=`Saving...`);let n=Array.from(document.querySelectorAll(`.char-select`)).map(e=>e.value),r=E(`characters`),i=n.filter(e=>{let t=r.find(t=>t.name===e);return t&&!X(t)});if(i.length>0&&!await k(`Unowned Heroes Selected`,`You have selected unowned heroes: ${i.join(`, `)}. Do you want to proceed?`)){e&&(e.disabled=!1,e.innerHTML=t);return}let a=document.querySelectorAll(`.char-select`),o=[],s=[],c=new Map(Array.from(a).map(e=>[parseInt(e.dataset.player),e.value])),l=E(`selectedGameType`),{data:u,error:d}=await T(E(`currentUser`).id,l);if(d)return e&&(e.disabled=!1,e.innerHTML=t),alert(`Error creating game: `+d.message);let f={},p=E(`teamAssignments`);if(st(l)&&p){let n=E(`currentUser`).id,{data:r,error:i}=await re([{game_id:u.id,team_label:`A`,last_updated_by:n},{game_id:u.id,team_label:`B`,last_updated_by:n}]);if(i)return e&&(e.disabled=!1,e.innerHTML=t),alert(`Error creating teams: `+i.message);let a=ft(),o=Object.fromEntries(r.map(e=>[e.team_label,e.id]));p.teamA.forEach(e=>{f[a[e]||e]=o.A}),p.teamB.forEach(e=>{f[a[e]||e]=o.B})}r.forEach(e=>{c.forEach((t,n)=>{if(t===e.name&&s.push({game_id:u.id,player_id:`p${n+1}`,hero_id:e.id,is_winner:null,team_id:f[`p${n+1}`]||null,last_updated_by:E(`currentUser`).id}),n>=4)return;let r=t===e.name?20:(e.weights[n]||250)+10;o.push({hero_id:e.id,player_id:`p${n+1}`,weight:r,last_updated_by:E(`currentUser`).id})})});let{error:m}=await ie(s);if(m)return e&&(e.disabled=!1,e.innerHTML=t),alert(`Error logging game participants: `+m.message);let{error:h}=await ae(o);if(h)return e&&(e.disabled=!1,e.innerHTML=t),alert(`Error saving results: `+h.message);await $();let g=document.getElementById(`action-buttons`);g&&(g.style.display=`none`),nt(),gt();let _=document.getElementById(`results`);_&&(_.innerHTML=`
            <p style="color:#28a745; text-align:center; font-weight:bold;">
                Session Logged! Game record created and stats updated.
            </p>`),D(`isRollActive`,!1)}function Tn(){let e=document.getElementById(`results`);e&&(e.innerHTML=``);let t=document.getElementById(`action-buttons`);t&&(t.style.display=`none`);let n=E(`scrambleIntervals`);n&&(Object.keys(n).forEach(e=>{n[e]&&clearInterval(n[e])}),D(`scrambleIntervals`,{})),D(`activeDraftOrder`,[]),D(`activeDraftStep`,0),D(`selectedDraftHeroes`,{}),D(`activeDraftCandidates`,{}),nt(),Ne(),_t(),D(`isRollActive`,!1)}function En(e){O(`stagedBannedHeroIds`,`toggle`,e),pn()}function Dn(e){D(`stagedBanSearchQuery`,e),pn()}function On(){fn()}function kn(){let e=document.getElementById(`results`);e&&(e.innerHTML=``);let t=E(`selectedDraftHeroes`);[...E(`activeDraftOrder`)].sort((e,t)=>e-t).forEach(e=>{let n=t[e];n&&on(e,n)})}function An(){let e=E(`activeDraftStep`),t=E(`activeDraftOrder`),n=E(`characters`),r=E(`bannedHeroIds`),i=E(`selectedDraftHeroes`);if(e>=t.length){kn(),it(`confirmation`),Cn();let e=document.getElementById(`action-buttons`);Ri()?e&&(e.style.display=`flex`):(nt(),gt()),D(`isRollActive`,!0);return}let a=t[e];sn(t,e),dn(a,null);let o=Object.values(i).map(e=>e?.name),s=n.filter(e=>X(e)&&!r.has(e.id)&&!o.includes(e.name)),c=E(`draftCount`),l=Math.min(c,s.length);cn(a,l),Mn(a,s,l),setTimeout(()=>{let e=jn(a,s);E(`activeDraftCandidates`)[a]=e,Nn(a),ln(a,e)},1e3)}function jn(e,t){let n=[],r=[...t],i=E(`draftCount`),a=Math.min(i,r.length);for(let t=0;t<a;t++){let t=null;if(e>=4){let e=Math.floor(Math.random()*r.length);t=r[e],r.splice(e,1)}else{let n=r.filter(t=>t.weights[e]>0);if(n.length===0){if(r.length>0){let e=Math.floor(Math.random()*r.length);t=r[e],r.splice(e,1)}}else{let i=n.reduce((t,n)=>t+Q(n,e),0),a=Math.random()*i;for(let i of n){let n=Q(i,e);if(a<n){t=i,r.splice(r.findIndex(e=>e.name===i.name),1);break}a-=n}}}t&&n.push(t)}return n}function Mn(e,t,n){t.length===0||n===0||me(`scrambleIntervals`,e,setInterval(()=>{for(let r=0;r<n;r++){let n=t[Math.floor(Math.random()*t.length)];if(!n)continue;let i=document.getElementById(`draft-card-img-${e}-${r}`),a=document.getElementById(`draft-card-name-${e}-${r}`),o=document.getElementById(`draft-card-group-${e}-${r}`);if(i&&(i.src=J(n.slug)),a){let e=``;for(let t=0;t<6;t++)e+=`ABCDEFGHIJKLMNOPQRSTUVWXYZ`[Math.floor(Math.random()*26)];a.innerText=e}o&&(o.innerText=n.group||``)}},70))}function Nn(e){let t=E(`scrambleIntervals`);t[e]&&(clearInterval(t[e]),me(`scrambleIntervals`,e,void 0))}function Pn(e,t){let n=(E(`activeDraftCandidates`)[e]||[]).find(e=>String(e.id)===String(t));if(!n)return;let r=E(`selectedDraftHeroes`);if(r[e]?.id===n.id){r[e]=null,un(e,null),dn(e,null);return}r[e]=n,un(e,n.id),dn(e,n)}function Fn(e){E(`selectedDraftHeroes`)[e]&&(D(`activeDraftStep`,E(`activeDraftStep`)+1),An())}function In(e){D(`stagedGamesWinnerOnly`,e),V()}function Ln(){D(`currentDrawerMode`,`history-filter`),D(`stagedSelectedGamePlayerIndex`,E(`selectedGamePlayerIndex`)),D(`stagedGamesWinnerOnly`,E(`gamesWinnerOnly`)),D(`stagedGamesUseHistorical`,E(`gamesUseHistorical`)),Dt()}function Rn(){D(`stagedFilterDataHistories`,new Set(E(`activeFilterDataHistories`))),D(`stagedFilterPlayers`,new Set(E(`activeFilterPlayers`))),D(`stagedFilterComplexities`,new Set(E(`activeFilterComplexities`))),D(`stagedFilterGroups`,new Set(E(`activeFilterGroups`))),D(`stagedOwnershipFilter`,E(`activeOwnershipFilter`)),kt()}function zn(e){D(`stagedOwnershipFilter`,e),Ot(),Bt()}function Bn(e=null,t=!1){At(e,t)}function Vn(e){let t=e.getAttribute(`data-type`),n=e.value,r=e.checked;if(t===`data-history`)O(`stagedFilterDataHistories`,r?`add`:`delete`,n);else if(t===`player`)O(`stagedFilterPlayers`,r?`add`:`delete`,n);else if(t===`complexity`){let e=Number(n);O(`stagedFilterComplexities`,r?`add`:`delete`,e)}else t===`group`&&O(`stagedFilterGroups`,r?`add`:`delete`,n);Bt(),Vt()}function Hn(){E(`stagedFilterDataHistories`).clear(),E(`stagedFilterPlayers`).clear(),E(`stagedFilterComplexities`).clear(),E(`stagedFilterGroups`).clear(),D(`stagedOwnershipFilter`,`all`),document.querySelectorAll(`#filter-drawer-left input[type="checkbox"]`).forEach(e=>{e.checked=!1}),Ot(),Bt(),Vt()}function Un(){D(`activeFilterDataHistories`,new Set(E(`stagedFilterDataHistories`))),D(`activeFilterPlayers`,new Set(E(`stagedFilterPlayers`))),D(`activeFilterComplexities`,new Set(E(`stagedFilterComplexities`))),D(`activeFilterGroups`,new Set(E(`stagedFilterGroups`))),D(`dbUseHistorical`,!E(`activeFilterDataHistories`).has(`Normal only`)||E(`activeFilterDataHistories`).has(`Historical only`));let e=E(`stagedOwnershipFilter`);D(`activeOwnershipFilter`,e),ti(e),Bn(null,!0),H(),er(),ur()}function Wn(){let e=document.getElementById(`hero-search`)?.value.toLowerCase()||``,t=E(`stagedOwnershipFilter`),n=t===`owned`||t===`all`,r=t===`unowned`||t===`all`,i=E(`characters`),a=E(`games`),o=E(`stagedFilterComplexities`),s=E(`stagedFilterGroups`),c=E(`stagedFilterDataHistories`),l=E(`stagedFilterPlayers`);return i.filter(t=>{let i=!0;o.size>0&&(i=o.has(Number(t.complexity)));let u=!0;s.size>0&&(u=s.has(t.group_id));let d=!0,f=c.has(`Normal only`),p=c.has(`Historical only`);f&&!p?d=a.filter(e=>e.game_players.some(e=>e.hero_id===t.id)).some(e=>!e.is_historical):p&&!f&&(d=a.filter(e=>e.game_players.some(e=>e.hero_id===t.id)).some(e=>e.is_historical));let m=!0;l.size>0&&(m=a.filter(e=>e.game_players.some(e=>e.hero_id===t.id)).some(e=>e.game_players.some(e=>e.hero_id===t.id&&l.has(e.player_id))));let h=X(t)&&n||!X(t)&&r;return pr(t,e)&&i&&u&&d&&m&&h}).length}function B(e=null,t=!1){jt(e,t)}function V(){Ut()}function Gn(e){E(`stagedSelectedGamePlayerIndex`)===e?D(`stagedSelectedGamePlayerIndex`,null):D(`stagedSelectedGamePlayerIndex`,e),E(`stagedSelectedGamePlayerIndex`)===null&&D(`stagedGamesWinnerOnly`,!1),V()}function Kn(e){D(`stagedGamesUseHistorical`,e),V()}function qn(e){e===`name`?D(`stagedSort`,`name`):e===`group`?D(`stagedSort`,`group`):e===`probability`?D(`stagedSort`,`w${E(`stagedSortPlayerIndex`)}`):e===`lastPlayed`&&D(`stagedSort`,`d${E(`stagedSortPlayerIndex`)}`),D(`stagedSortAsc`,e===`name`||e===`group`),Wt(),Gt()}function Jn(e){D(`stagedSortPlayerIndex`,e);let t=E(`stagedSort`);t.startsWith(`w`)?D(`stagedSort`,`w${e}`):t.startsWith(`d`)&&D(`stagedSort`,`d${e}`),Gt()}function Yn(e){let t=E(`stagedPlayerIndices`),n=t.indexOf(e);n>-1?t.splice(n,1):t.push(e),D(`stagedPlayerIndices`,t),V()}function Xn(e){e===`all`?D(`stagedLevels`,E(`stagedLevels`).size===6?new Set:new Set([1,2,3,4,5,6])):O(`stagedLevels`,`toggle`,e),Kt(),Yt()}function Zn(e){let t=E(`groups`);e===`all`?E(`stagedGroups`).size===t.length?O(`stagedGroups`,`clear`):t.forEach(e=>O(`stagedGroups`,`add`,e.id)):O(`stagedGroups`,`toggle`,e),Kt(),Yt()}function Qn(){let e=E(`groups`),t=E(`currentDrawerMode`);t===`sort-filter`?(D(`stagedSort`,`name`),D(`stagedSortAsc`,!0),D(`stagedSortPlayerIndex`,0),D(`stagedLevels`,new Set([1,2,3,4,5,6])),D(`stagedGroups`,new Set(e.map(e=>e.id))),V()):t===`columns`?(D(`stagedPlayerIndices`,[0,1,2,3]),D(`stagedUseHistorical`,!0),V()):t===`history-filter`?(D(`stagedSelectedGamePlayerIndex`,null),D(`stagedGamesWinnerOnly`,!1),D(`stagedGamesUseHistorical`,!0),V()):t===`roll-settings`&&(D(`stagedBannedHeroIds`,new Set),D(`stagedBanSearchQuery`,``),V())}function $n(){let e=E(`currentDrawerMode`);e===`sort-filter`?(D(`currentSort`,E(`stagedSort`)),D(`sortAsc`,E(`stagedSortAsc`)),D(`currentSortPlayerIndex`,E(`stagedSortPlayerIndex`)),D(`activeLevels`,new Set(E(`stagedLevels`))),D(`activeGroups`,new Set(E(`stagedGroups`))),er(),B(null,!0),H()):e===`columns`?(D(`activePlayerIndices`,[...E(`stagedPlayerIndices`)]),D(`dbUseHistorical`,E(`stagedUseHistorical`)),er(),B(null,!0),H()):e===`history-filter`?(D(`selectedGamePlayerIndex`,E(`stagedSelectedGamePlayerIndex`)),D(`gamesWinnerOnly`,E(`stagedGamesWinnerOnly`)),D(`gamesUseHistorical`,E(`stagedGamesUseHistorical`)),tr(),B(null,!0),G()):e===`roll-settings`&&(D(`bannedHeroIds`,new Set(E(`stagedBannedHeroIds`))),localStorage.setItem(`bannedHeroIds`,JSON.stringify(Array.from(E(`bannedHeroIds`)))),On(),B(null,!0))}function er(){Nt()}function tr(){Pt()}function nr(e){let t=E(`currentSort`),n=E(`sortAsc`);t===e?D(`sortAsc`,!n):(D(`currentSort`,e),D(`sortAsc`,!0)),rr(),H()}function rr(){It()}function ir(e){Rt(e)}function ar(){zt()}function or(e,t){D(`currentSort`,e),D(`sortAsc`,t),(e.startsWith(`w`)||e.startsWith(`d`))&&D(`currentSortPlayerIndex`,parseInt(e.substring(1))),ar(),rr(),H()}function sr(){let e=document.getElementById(`hero-search`),t=document.getElementById(`clear-search`);e&&t&&t.classList.toggle(`hidden`,e.value.trim().length===0)}function cr(){let e=document.getElementById(`hero-search`);e&&(e.value=``,e.focus()),sr(),lr()}function lr(){H(),ur()}function ur(){Xt()}function dr(e,t){if(e===`data-history`)O(`activeFilterDataHistories`,`delete`,t),D(`dbUseHistorical`,!E(`activeFilterDataHistories`).has(`Normal only`)||E(`activeFilterDataHistories`).has(`Historical only`));else if(e===`player`)O(`activeFilterPlayers`,`delete`,t);else if(e===`complexity`)O(`activeFilterComplexities`,`delete`,t);else if(e===`group`)O(`activeFilterGroups`,`delete`,t);else if(e===`ownership`){D(`activeOwnershipFilter`,`all`),D(`stagedOwnershipFilter`,`all`);let e=document.getElementById(`db-show-owned`),t=document.getElementById(`db-show-not-owned`);e&&(e.checked=!0),t&&(t.checked=!0)}if(e!==`ownership`){let n=document.querySelector(`#filter-drawer-left input[value="${t}"][data-type="${e}"]`);n&&(n.checked=!1)}H(),ur(),er()}function fr(){cr()}function H(){Zt()}function pr(e,t){if(!t)return!0;let n=t.trim().toLowerCase();return(e.name||``).toLowerCase().includes(n)||(e.group||``).toLowerCase().includes(n)}var mr={duel:`1v1 Duel`,"2v2":`Teams 2v2`,"3v3":`Teams 3v3`,ffa:`Free For All`,koth:`King of the Hill`};function hr(e){if(!e)return``;let t=j[e===`2v2`||e===`3v3`?`teams`:e];return t?`<span class="game-card-type-icon" title="${Ke[e]||e}">${t}</span>`:``}function gr(e){return e-4+1}var _r=null,U=()=>(_r||={buildInfoDiv:document.getElementById(`admin-build-info`),changelogModal:document.getElementById(`changelog-modal`),changelogContainer:document.getElementById(`changelog-container`),whatsNewModal:document.getElementById(`whats-new-modal`),whatsNewContainer:document.getElementById(`whats-new-container`),collectionContainer:document.getElementById(`collectionContainer`),collectionCountLabel:document.getElementById(`collection-count-stats`),heroForm:document.getElementById(`heroForm`),addHeroBtn:document.getElementById(`addHeroBtn`),groupSelect:document.getElementById(`charGroup`),formTitle:document.getElementById(`formTitle`),charNameInput:document.getElementById(`charName`),charSlugInput:document.getElementById(`charSlug`),charComplexitySelect:document.getElementById(`charComplexity`),groupsListContainer:document.getElementById(`groupsListContainer`),heroesListContainer:document.getElementById(`heroesListContainer`),playersListContainer:document.getElementById(`playersListContainer`),usersListContainer:document.getElementById(`usersListContainer`),collectionsListContainer:document.getElementById(`collectionsListContainer`),gamesListContainer:document.getElementById(`gamesContainer`),winnerModal:document.getElementById(`winner-modal`),winnerContainer:document.getElementById(`winner-selection-container`),confirmWinnerBtn:document.getElementById(`confirm-winner-btn`),groupForm:document.getElementById(`groupForm`),addGroupBtn:document.getElementById(`addGroupBtn`)},_r);function vr(){let t=U();if(!t.buildInfoDiv)return;let n=window.location.hostname,r=`Localhost`;n.includes(`github.io`)?r=`GitHub Pages`:n.includes(`workers.dev`)&&(r=`Cloudflare Workers`);let i=e?`Production`:`Development`,a=e?`Supabase PROD`:`Supabase DEV`,o=e?`main`:`dev/local`;t.buildInfoDiv.innerHTML=`
    ...
    `}function yr(){let e=U(),t=E(`cachedChangelog`);!t||!e.changelogContainer||!e.changelogModal||(e.changelogContainer.innerHTML=t.map(e=>`
        <div>
            <h3>v${e.version}</h3>
            <ul>
    ...
    `).join(``),t.whatsNewModal.style.display=`flex`,document.body.style.overflow=`hidden`)}function Sr(){let e=U();e.whatsNewModal&&(e.whatsNewModal.style.display=`none`),document.body.style.overflow=`auto`}var Cr={roll:{title:`Randomizer`},database:{title:`Heroes`},history:{title:`History`},collection:{title:`Collection`},admin:{title:`Admin`}};function wr(e){let t=Cr[e];if(!t)return;let n=document.getElementById(`header-section-title`);n&&(n.innerText=t.title)}function Tr(e){if(e===`admin`&&!K())return;let t={roll:`rollSection`,database:`dbSection`,history:`gamesSection`,collection:`collectionSection`,admin:`adminSection`},n=t[e];n&&(Object.values(t).forEach(e=>{let t=document.getElementById(e);t&&(e===n?t.classList.remove(`hidden`):t.classList.add(`hidden`))}),wr(e),e===`database`?setTimeout(Ft,50):e===`history`?Gr():e===`collection`&&Or())}function Er(e,t){let n=document.getElementById(t),r=(e.currentTarget.closest(`.panel-header`)||e.currentTarget).querySelector(`.panel-toggle`);if(!n||!r)return;let i=n.classList.toggle(`hidden`);r.classList.toggle(`open`,!i),r.setAttribute(`aria-expanded`,String(!i))}function Dr(e){let t=e.closest(`.hero-item`),n=e.querySelector(`.panel-toggle`);if(!t||!n)return;let r=t.classList.toggle(`collapsed`);n.classList.toggle(`open`,!r),n.setAttribute(`aria-expanded`,String(!r))}function Or(){let e=U();if(!e.collectionContainer)return;let t=E(`characters`),n=E(`groups`),r=E(`currentUser`),i=E(`expandedCollectionGroups`),a=t.length,o=t.filter(E(`isHeroOwned`)||(e=>e.is_owned)).length;e.collectionCountLabel&&(e.collectionCountLabel.innerText=`Owned ${o} of ${a} heroes`);let s=[...n].sort((e,t)=>{let n=e.order_index??2**53-1,r=t.order_index??2**53-1;return n===r?e.name.localeCompare(t.name):n-r}),c=r?``:`disabled`,l=[];e.collectionContainer.innerHTML=s.map(e=>{let n=t.filter(t=>t.group_id===e.id).sort((e,t)=>e.name.localeCompare(t.name));if(n.length===0)return``;let r=n.every(E(`isHeroOwned`)||(e=>e.is_owned)),a=n.map(e=>{let t=e.is_owned;return`
        ...
        `}).join(``),l.forEach(e=>{let t=document.getElementById(`owned-group-${e}`);t&&(t.indeterminate=!0)})}function kr(){let e=U();e.charNameInput&&(e.charNameInput.value=``),e.charSlugInput&&(e.charSlugInput.value=``),e.groupSelect&&(e.groupSelect.value=``),e.charComplexitySelect&&(e.charComplexitySelect.value=``),e.formTitle&&(e.formTitle.innerText=`Add New Hero`),e.heroForm&&e.addHeroBtn&&(e.heroForm.classList.add(`hidden`),e.addHeroBtn.innerText=`Add Hero`)}function Ar(){let e=U();if(!e.heroForm||!e.addHeroBtn)return;let t=e.heroForm.classList.toggle(`hidden`);e.addHeroBtn.innerText=t?`Add Hero`:`Hide Hero Form`,!t&&e.charNameInput&&e.charNameInput.focus()}function jr(){let e=U();if(!e.groupSelect)return;let t=E(`groups`).map(e=>`<option value="${e.id}">${e.name}</option>`).join(``);e.groupSelect.innerHTML=`<option value="">-- Select Group --</option>`+t}function Mr(){let e=U();if(!e.groupsListContainer)return;let t=E(`groups`);if(t.length===0){e.groupsListContainer.innerHTML=`<p style="opacity: 0.6; font-style: italic;">No groups yet. Create one above.</p>`;return}let n=t.map(e=>`
    ...
    `).join(``);e.groupsListContainer.innerHTML=n}function Nr(){let e=U();if(!e.heroesListContainer)return;let t=E(`characters`),n=E(`editIndex`),r=E(`groups`);if(t.length===0){e.heroesListContainer.innerHTML=`<p style="opacity: 0.6; font-style: italic;">No heroes yet. Add one above.</p>`;return}let i=t.map((e,t)=>{let i=n===t,a=K()?`<button class="btn-save btn-inline" data-action="edit-hero" data-hero-idx="${t}">Edit</button>`:``,o=K()?`<button class="btn-cancel btn-inline" data-action="delete-hero" data-hero-id="${e.id}">Delete</button>`:``,s=r.map(t=>`<option value="${t.id}" ${t.id===e.group_id?`selected`:``}>${Z(t.name)}</option>`).join(``);return`
            ...
            </div>`}).join(``);e.heroesListContainer.innerHTML=i,n===-1?e.heroesListContainer.classList.remove(`group-edit-active`):e.heroesListContainer.classList.add(`group-edit-active`)}function Pr(e){let t=document.getElementById(`groupEditPanel-${e}`),n=document.getElementById(`groupRow-${e}`);t&&t.classList.add(`hidden`),n&&n.classList.remove(`editing`);let r=U().groupsListContainer;r&&(r.querySelectorAll(`.group-row.editing`).length>0||r.classList.remove(`group-edit-active`))}function Fr(){let e=document.getElementById(`groupName`),t=document.getElementById(`groupOrder`),n=document.getElementById(`groupYear`);e&&(e.value=``),t&&(t.value=``),n&&(n.value=``);let r=U().groupForm,i=U().addGroupBtn;r&&i&&(r.classList.add(`hidden`),i.innerText=`Add Group`)}function Ir(){let e=U().groupForm,t=U().addGroupBtn;if(!e||!t)return;let n=e.classList.toggle(`hidden`);if(t.innerText=n?`Add Group`:`Hide Group Form`,!n){let e=document.getElementById(`groupName`);e&&e.focus()}}function Lr(){let e=U();if(!e.playersListContainer)return;let t=E(`players`);if(t.length===0){e.playersListContainer.innerHTML=`<p style="opacity: 0.6; font-style: italic;">No players loaded.</p>`;return}let n=t.map((e,t)=>{let n=Bi(e);return`
    ...
    `,t.winnerModal.style.display=`flex`,document.body.style.overflow=`hidden`}function Wr(){let e=U();e.winnerModal&&(e.winnerModal.style.display=`none`),document.body.style.overflow=`auto`}function Gr(){let e=U();if(!e.gamesListContainer)return;let t=E(`games`);E(`players`);let n=E(`NAMES`),r=E(`expandedGameIds`),i=E(`selectedGamePlayerIndex`),a=E(`gamesWinnerOnly`),o=E(`gamesUseHistorical`),s=E(`gamesHistoryStyle`)||`gorgeous`,c=document.getElementById(`games-search`),l=c?c.value.toLowerCase().trim():``,u=s===`gorgeous`,d=e=>{let n=document.getElementById(`game-count-stats`);n&&(n.innerText=`Showing ${e} of ${t?t.filter(e=>u?!e.is_historical:o||!e.is_historical).length:0} games`)},f=``;if(K()&&(f=`
        ...
        `),!t||t.length===0){e.gamesListContainer.innerHTML=f+`<p style="opacity: 0.7; font-style: italic; text-align: center; padding: 20px;">No games played yet.</p>`,d(0);return}let p=t.filter(e=>{if(u&&e.is_historical||!u&&!o&&e.is_historical)return!1;let t=!0;return i!==null&&(t=e.game_players.some(e=>{let t=parseInt(e.player_id.substring(1))-1,n=!1;return i>=0&&i<4?n=t===i:i===4&&(n=t>=4),n&&a?e.is_winner===!0:n})),!(!t||l&&!(e.game_players||[]).map(e=>e.heroes?.name||``).join(` `).toLowerCase().includes(l))});if(p.length===0){e.gamesListContainer.innerHTML=f+`<p style="opacity: 0.7; font-style: italic; text-align: center; padding: 20px;">No matches found matching filter criteria.</p>`,d(0);return}d(p.length),u?e.gamesListContainer.innerHTML=f+p.map(e=>{let a=e.played_at||``;a&&!a.includes(`T`)&&(a=a.replace(` `,`T`)),a&&!a.includes(`Z`)&&!a.includes(`+`)&&(a+=`Z`);let o=new Date(a).toLocaleDateString(void 0,{dateStyle:`medium`}),s=new Date(a).toLocaleTimeString(void 0,{timeStyle:`short`}),c=e.game_type===`2v2`||e.game_type===`3v3`,u=hr(e.game_type),d=e.game_type?`<div class="game-card-type-badge">${mr[e.game_type]||e.game_type}</div>`:``,f=e.game_players.filter(e=>e.is_winner===!0),p=e.game_players.filter(e=>e.is_winner===!1),m=f.length===0&&p.length>0&&p.length===e.game_players.length,h=f.length===0&&!m,g=r.has(e.id)?`expanded`:``,_=h?`in-progress`:``,v=``;f.length>0&&f[0].heroes?.slug&&(v=`<img src="${J(f[0].heroes.slug)}" class="game-card-bg-img" alt="">`);let y={};e.game_players.forEach(e=>{let t=parseInt(e.player_id.substring(1))-1,r;t>=4?r=`Invitee ${gr(t)}`:(r=n[t]||`Unknown`,r.toLowerCase().startsWith(`player `)&&r.length>7&&(r=`P`+r.substring(7))),y[e.player_id]=r});let b=Object.values(y).map(e=>e.charAt(0).toUpperCase()),x={};e.game_players.forEach(e=>{let t=y[e.player_id],n=t.charAt(0).toUpperCase(),r=b.filter(e=>e===n).length,i=n;r>1&&t.length>1&&(i=n+t.charAt(1).toLowerCase()),x[e.player_id]=i});let S=[...e.game_players].sort((e,t)=>e.is_winner&&!t.is_winner?-1:!e.is_winner&&t.is_winner?1:0).map(e=>{let t=parseInt(e.player_id.substring(1))-1,n=e.heroes?.slug||``,r=e.heroes?.name||`Unknown`,i=e.is_winner===!0,a=i?`winner-highlight`:``,o=i?`<span class="mini-winner-trophy">🏆</span>`:``,s=x[e.player_id];return`
                        ...
                        `}).join(``),ee=h?`<span class="game-card-status-badge">In Progress</span>`:``,te=m?`<div class="player-plate-draw-badge">DRAW</div>`:``,ne=`
                ...
                </div>`,C=K()||e.last_updated_by===E(`currentUser`)?.id?`
                ...
                `:``,w=e=>{let r=parseInt(e.player_id.substring(1))-1,a=r>=4?`Invitee ${gr(r)}`:n[r],o=e.heroes?.name||`Unknown`,s=e.heroes?.slug||``,c=!!(l&&o.toLowerCase().includes(l)),u=!1;i!==null&&(i>=0&&i<4?u=r===i:i===4&&(u=r>=4));let d=`draw`;d=f.length>0?e.is_winner?`winner`:`loser`:m?`draw`:h?`in-progress`:e.is_winner===!1?`loser`:`draw`;let p=``;(c||u)&&(p=`box-shadow: 0 0 8px var(--accent), 0 0 20px color-mix(in srgb, var(--accent) 50%, transparent);`);let g=e.is_winner?`<div class="player-plate-trophy">🏆</div>`:``,_=m?`<div class="player-plate-draw-badge">DRAW</div>`:``,v=0,y=0,b=E(`gamesUseHistorical`);t.forEach(t=>{!b&&t.is_historical||t.game_players.forEach(t=>{t.player_id===e.player_id&&t.hero_id===e.hero_id&&(v++,t.is_winner&&y++)})});let x=v>0?(y/v).toFixed(3):`.000`,S=x.startsWith(`0`)?x.substring(1):x;return`
                        <div class="player-plate-wrapper">
                            ...
                            <div class="player-plate-stats-below">
                                ...
                                <span class="player-plate-winner-pct">( ${S})</span>
                            </div>
                        </div>`},T;if(c){let t=e.game_players.filter(e=>e.teams?.team_label===`A`),n=e.game_players.filter(e=>e.teams?.team_label===`B`);T=`
                        <div class="team-block">
                            <div class="team-block-label">Team A</div>
                            <div class="player-responsive-grid">${t.map(w).join(``)}</div>
                        </div>
                        <div class="team-block">
                            <div class="team-block-label">Team B</div>
                            <div class="player-responsive-grid">${n.map(w).join(``)}</div>
                        </div>
                    `}else T=`<div class="player-responsive-grid">${e.game_players.map(w).join(``)}</div>`;return`
                ...
                </div>`}).join(``)}function Kr(){vr()}function qr(){yr()}function Jr(){br()}function Yr(e){xr(e),localStorage.setItem(`lastSeenVersion`,e[0].version)}function Xr(){Sr()}function Zr(e){Tr(e)}function Qr(e,t){Er(e,t)}function $r(e){Dr(e)}function ei(e,t){t.target.tagName===`INPUT`||t.target.tagName===`LABEL`||t.target.closest(`label`)||(O(`expandedCollectionGroups`,`toggle`,e),W())}function ti(e){let t=document.getElementById(`db-show-owned`),n=document.getElementById(`db-show-not-owned`);t&&n&&(e===`owned`?(t.checked=!0,n.checked=!1):e===`unowned`?(t.checked=!1,n.checked=!0):(t.checked=!0,n.checked=!0)),H()}function W(){Or()}async function ni(e,t){if(!E(`currentUser`)){alert(`Please log in to manage your collection.`);return}let n=E(`characters`).find(t=>t.id===e);n&&(n.is_owned=t),W(),H(),z();let r=document.getElementById(`admin-owned-${E(`currentUser`).id}-${e}`);r&&(r.checked=t);let{error:i}=await x(E(`currentUser`).id,e,t);i&&(alert(`Error updating ownership: `+i.message),n&&(n.is_owned=!t),W(),z(),H(),r&&(r.checked=!t))}async function ri(e,t){if(!E(`currentUser`)){alert(`Please log in to manage your collection.`);return}let n=E(`characters`);n.forEach(n=>{if(n.group_id===e){n.is_owned=t;let e=document.getElementById(`admin-owned-${E(`currentUser`).id}-${n.id}`);e&&(e.checked=t)}}),W(),H(),z();let{error:r}=await S(n.filter(t=>t.group_id===e).map(e=>({user_id:E(`currentUser`).id,hero_id:e.id,is_owned:t})));r&&(alert(`Error updating group ownership: `+r.message),n.forEach(n=>{n.group_id===e&&(n.is_owned=!t)}),W(),z(),H())}async function ii(){let e=document.getElementById(`charName`).value.trim(),t=document.getElementById(`charGroup`).value,n=document.getElementById(`charSlug`).value.trim(),r=document.getElementById(`charComplexity`).value.trim();if(!e)return alert(`Name is required`);if(!t)return alert(`Group is required`);let{error:i}=await ee({name:e,slug:n,complexity:r?parseInt(r):null,group_id:t,last_updated_by:E(`currentUser`).id});if(i)return alert(`Error saving: `+i.message);await $(),si()}function ai(e){D(`editIndex`,e),di(),document.getElementById(`adminSection`).classList.contains(`hidden`)&&Zr(`admin`);let t=E(`characters`),n=document.getElementById(`heroEditPanel-${t[e]?.id}`);n&&!oi(n)&&n.scrollIntoView({behavior:`smooth`,block:`nearest`})}function oi(e){let t=e.getBoundingClientRect();return t.top>=0&&t.left>=0&&t.bottom<=(window.innerHeight||document.documentElement.clientHeight)}function si(){D(`editIndex`,-1),kr()}function ci(){Ar()}function li(){jr()}function ui(){Mr()}function di(){Nr()}function fi(){D(`editIndex`,-1),di()}async function pi(e,t){let n=document.getElementById(`heroName-${t}`).value.trim(),r=document.getElementById(`heroGroup-${t}`).value,i=document.getElementById(`heroSlug-${t}`).value.trim(),a=document.getElementById(`heroComplexity-${t}`).value.trim();if(!n)return alert(`Name is required`);if(!r)return alert(`Group is required`);let{error:o}=await te({id:e,name:n,slug:i,complexity:a?parseInt(a):null,group_id:r,last_updated_by:E(`currentUser`).id});if(o)return alert(`Error saving: `+o.message);D(`editIndex`,-1),await $()}async function mi(e){if(!await k(`Delete Hero`,`Delete this hero? This action cannot be undone.`))return;let{error:t}=await ne(e);if(t)return alert(`Error deleting hero: `+t.message);await $()}async function hi(){let e=document.getElementById(`groupName`).value.trim(),t=document.getElementById(`groupOrder`).value.trim(),n=document.getElementById(`groupYear`).value.trim();if(!e)return alert(`Group name is required`);let{error:r}=await C({name:e,order_index:t?parseInt(t):null,year:n?parseInt(n):null,is_active:!0});if(r)return alert(`Error saving group: `+r.message);yi(),$()}function gi(e){let t=E(`groups`).find(t=>t.id===e);if(!t)return;let n=document.getElementById(`groupsListContainer`);n&&(n.classList.add(`group-edit-active`),n.querySelectorAll(`.group-row`).forEach(e=>e.classList.remove(`editing`)));let r=document.getElementById(`groupEditPanel-${e}`),i=document.getElementById(`groupRow-${e}`);!r||!i||(i.classList.add(`editing`),document.getElementById(`groupName-${e}`).value=t.name,document.getElementById(`groupOrder-${e}`).value=t.order_index||``,document.getElementById(`groupYear-${e}`).value=t.year||``,r.classList.remove(`hidden`))}function _i(e){Pr(e)}async function vi(e){let t=document.getElementById(`groupName-${e}`).value.trim(),n=document.getElementById(`groupOrder-${e}`).value.trim(),r=document.getElementById(`groupYear-${e}`).value.trim();if(!t)return alert(`Group name is required`);let{error:i}=await C({id:e,name:t,order_index:n?parseInt(n):null,year:r?parseInt(r):null,is_active:!0});if(i)return alert(`Error saving group: `+i.message);$()}function yi(){Fr()}function bi(){Ir()}function xi(){Lr()}function Si(e){let t=E(`players`).find(t=>t.id===e);if(!t)return;let n=document.getElementById(`playersListContainer`);n&&(n.classList.add(`player-edit-active`),n.querySelectorAll(`.player-admin-row`).forEach(e=>e.classList.remove(`editing`)));let r=document.getElementById(`playerEditPanel-${e}`),i=document.getElementById(`playerRow-${e}`);!r||!i||(i.classList.add(`editing`),document.getElementById(`playerName-${e}`).value=t.name,r.classList.remove(`hidden`))}function Ci(e){Rr(e)}async function wi(e){let t=document.getElementById(`playerName-${e}`).value.trim();if(!t)return alert(`Player name is required`);let{error:n}=await b(e,t);if(n)return alert(`Error saving player: `+n.message);let r=E(`players`),i=E(`NAMES`),a=r.findIndex(t=>t.id===e);a!==-1&&(r[a].name=t,i[a]=t),Ci(e),xi()}function Ti(){zr()}async function Ei(){let e=document.getElementById(`collectionsListContainer`);if(!e)return;e.innerHTML=`<p style="opacity: 0.7; font-style: italic; padding: 10px;">Loading collections...</p>`;let t=[];try{let{data:n,error:r}=await v();if(r){e.innerHTML=`<p style="color: var(--danger); padding: 10px;">Error loading collections: ${Z(r.message)}</p>`;return}t=n||[]}catch(t){e.innerHTML=`<p style="color: var(--danger); padding: 10px;">Error connecting to database: ${Z(t.message)}</p>`;return}let n=[],r=E(`players`);r.forEach(e=>{e.user_id&&n.push({user_id:e.user_id,name:e.name,isLinked:!0})}),t.forEach(e=>{n.some(t=>t.user_id===e.user_id)||n.push({user_id:e.user_id,name:`User (${e.user_id.substring(0,8)})`,isLinked:!1})});let i=E(`currentUser`);if(i&&!n.some(e=>e.user_id===i.id)){let e=r.find(e=>e.user_id===i.id),t=e?e.name:i.email?i.email.split(`@`)[0]:`Admin`;n.push({user_id:i.id,name:t,isLinked:!!e})}Br(n,t)}async function Di(e,t,n){let r=E(`currentUser`);if(e===r?.id){let e=E(`characters`).find(e=>e.id===t);e&&(e.is_owned=n),W(),H(),z()}let{error:i}=await x(e,t,n);if(i){if(alert(`Error updating user collection: `+i.message),e===r?.id){let e=E(`characters`).find(e=>e.id===t);e&&(e.is_owned=!n),W(),H(),z()}Ei()}}async function Oi(e){if(!await k(`Delete Group`,`Delete this group?`))return;let{error:t}=await w(e);if(t)return alert(`Error deleting group: `+t.message);yi(),$()}function G(){Gr()}function ki(){let e=document.getElementById(`games-search`),t=document.getElementById(`clear-games-search`);e&&t&&t.classList.toggle(`hidden`,e.value.trim().length===0),G()}function Ai(){let e=document.getElementById(`games-search`);e&&(e.value=``,e.focus()),ki()}function ji(e){O(`expandedGameIds`,`toggle`,e),G()}function Mi(){D(`gamesHistoryStyle`,(E(`gamesHistoryStyle`)||`gorgeous`)===`gorgeous`?`admin`:`gorgeous`),G()}function Ni(e){Ur(e)}function Pi(){Wr()}async function Fi(e){let t=document.querySelector(`input[name="winner-selection"]:checked`);if(!t)return alert(`Please select a winner.`);let n=t.value,r=document.getElementById(`confirm-winner-btn`);r.disabled=!0,r.innerText=`Saving...`;let i=E(`games`).find(t=>t.id===e),a=i?.game_type===`2v2`||i?.game_type===`3v3`;try{let{error:t}=a?await se(e,n,E(`currentUser`).id):await oe(e,n,E(`currentUser`).id);if(t)throw t;Pi(),await $()}catch(e){alert(`Error updating winner: `+e.message)}finally{r.disabled=!1,r.innerText=`Save Result`}}async function Ii(e){if(!await k(`Delete Game Record`,`Are you sure you want to delete this game record? This cannot be undone.`))return;let{error:t}=await ce(e);if(t)return console.error(`Error deleting game:`,t),alert(`Failed to delete game: `+t.message);await $()}function Li(){let e=!0,t=!0,n=E(`activeFilterDataHistories`),r=n.has(`Normal only`),i=n.has(`Historical only`);r&&!i?(e=!0,t=!1):i&&!r&&(e=!1,t=!0);let a=E(`characters`);a.forEach(e=>{e.playCount=[0,0,0,0],e.lastPlayed=[`Never`,`Never`,`Never`,`Never`],e.winCount=[0,0,0,0]});let o=E(`games`);o&&o.forEach(n=>{let r=!!n.is_historical;r&&!t||!r&&!e||n.game_players.forEach(e=>{let t=parseInt(e.player_id?.substring(1)||`0`,10)-1;if(t>=0&&t<4){let r=a.find(t=>t.id===e.hero_id);if(!r)return;if(r.playCount[t]++,e.is_winner&&r.winCount[t]++,r.lastPlayed[t]===`Never`){let e=n.played_at||``;e&&!e.includes(`T`)&&(e=e.replace(` `,`T`)),e&&!e.includes(`Z`)&&!e.includes(`+`)&&(e+=`Z`);let i=new Date(e);r.lastPlayed[t]=i.getFullYear()<2026?`Unknown`:i.toLocaleDateString(`en-CA`)}}})})}window.alert=function(e){he(e,e&&(e.toLowerCase().includes(`error`)||e.toLowerCase().includes(`failed`))?`error`:`warning`)};function K(){return E(`currentUser`)?.app_metadata?.role===`admin`}function Ri(){return!!E(`currentUser`)}function q(e){return E(`activeRollParticipants`).find(t=>t.pIdx===e)}var J=e=>e?`https://dice-throne.rulepop.com/heroes/${e}.webp`:``,Y=e=>`https://dice-throne.rulepop.com/#hero/${e}`,X=e=>!!e?.is_owned,Z=e=>String(e||``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#039;`),zi=e=>{if(!e)return`#ffffff`;if(e=e.trim(),e.startsWith(`#`))return e;let t=e.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);if(t){let e=parseInt(t[1],10),n=parseInt(t[2],10),r=parseInt(t[3],10);return`#${(1<<24|e<<16|n<<8|r).toString(16).slice(1)}`}return e},Bi=e=>e?.player_color?zi(e.player_color):zi(getComputedStyle(document.documentElement).getPropertyValue(`--${e?.id}`).trim()),Vi=(e,t)=>{document.documentElement.style.setProperty(`--${e}`,t)};function Q(e,t){return e.weights[t]/(e.playCount[t]*3+1)**2}function Hi(e,t){if(!e)return`0.00%`;let n=E(`characters`).filter(X).length;if(n===0)return`0.00%`;if(t>=4)return`${(100/n).toFixed(2)}%`;let r=0;if(E(`characters`).filter(X).forEach(e=>r+=Q(e,t)),r===0)return`0.00%`;let i=X(e),a=Q(e,t);return`${i?(a/r*100).toFixed(2):`0.00`}%`}async function Ui(e,t){let n=E(`players`).find(t=>t.id===e);if(!n)return;let r=Bi(n),i=zi(t.value);if(i.toLowerCase()===r.toLowerCase())return;if(!await k(`Change Player Color`,`Change ${n.name}'s color from ${r} to ${i}?`)){t.value=r;return}let{error:a}=await y(e,i);if(a){alert(`Error saving player color: `+a.message),t.value=r;return}let o=E(`players`).findIndex(t=>t.id===e);o!==-1&&(E(`players`)[o].player_color=i),Vi(e,i),xi()}function Wi(e){if(!e)return null;try{let t=e.trim();t&&!t.includes(`T`)&&(t=t.replace(` `,`T`)),t&&t.includes(`:`)&&!t.includes(`Z`)&&!t.includes(`+`)&&(t+=`Z`);let n=new Date(t);return isNaN(n.getTime())?null:n}catch{return null}}function Gi(e){if(!e||e===`Never`)return``;if(e===`Unknown`)return`Date unknown (historical)`;let t=Wi(e);if(!t)return``;try{let e=new Date;t.setHours(0,0,0,0),e.setHours(0,0,0,0);let n=e-t,r=Math.floor(n/(1e3*60*60*24));return r<0?``:r===0?`today`:r===1?`yesterday`:`${r} days ago`}catch{return``}}function Ki(e){let t=`⚫`;if(e&&e===`Unknown`)t=`🔴`;else if(e&&e!==`Never`&&e!==`Unknown`){let n=Wi(e);if(n)try{let e=new Date;n.setHours(0,0,0,0),e.setHours(0,0,0,0);let r=e-n,i=Math.floor(r/(1e3*60*60*24));t=i<=15?`🟢`:i<=60?`🟡`:`🔴`}catch{t=`⚪`}else t=`⚪`}return t}function qi(){let e=(e,t)=>{let n=document.getElementById(e);n&&n.addEventListener(`click`,t)};e(`roll-final-btn`,hn),e(`cancelBtn`,Tn),e(`confirmBtn`,wn),e(`clear-search`,cr),e(`hero-search-btn`,lr),e(`btn-trigger-sort`,ir),e(`btn-trigger-filter`,Rn),e(`clear-games-search`,Ai),e(`btn-trigger-games-filter`,Ln);let t=document.getElementById(`randomizer-setup`);t&&(t.addEventListener(`change`,e=>{let t=e.target.closest(`#invitee-zone input[data-invitee-id]`);if(t&&!t.checked)return ht(t.dataset.inviteeId);e.target.closest(`#player-toggle-zone-top, #invitee-zone`)&&yt()}),t.addEventListener(`click`,e=>{if(e.target.closest(`[data-action="add-invitee"]`))return mt();let t=e.target.closest(`[data-action="select-game-type"]`);if(t&&!t.disabled)return vt(t.dataset.type);if(e.target.closest(`[data-action="randomize-teams"]`))return St();let n=e.target.closest(`[data-action="initiate-team-swap"]`);if(n)return Ct(n.dataset.participantId,n.dataset.team);let r=e.target.closest(`[data-action="complete-team-swap"]`);if(r)return Tt(r.dataset.participantId);let i=e.target.closest(`[data-action="select-roll-mode"]`);if(i&&!i.disabled)return bt(i.dataset.mode);let a=e.target.closest(`[data-action="select-draft-count"]`);if(a)return xt(parseInt(a.dataset.count,10))}));let n=document.getElementById(`team-swap-scrim`);n&&n.addEventListener(`click`,()=>wt()),document.querySelectorAll(`.bottom-nav .nav-item`).forEach(e=>{e.addEventListener(`click`,t=>{t.preventDefault();let n=e.getAttribute(`data-section`);n&&Zr(n)})});let r=document.getElementById(`version-number`);r&&r.addEventListener(`click`,()=>{Le(),qr()});let i=document.querySelector(`.close-button`);i&&i.addEventListener(`click`,Jr),e(`whats-new-close`,Xr),e(`whats-new-got-it`,Xr),e(`winner-close`,Pi),e(`winner-cancel`,Pi),e(`login-close`,Fe),e(`forgot-password-btn`,ze),e(`update-password-close`,Ve),e(`hero-select-close`,_n),e(`header-avatar-btn`,Ie),e(`account-close`,Le);let a=document.getElementById(`hero-search`);a&&(a.addEventListener(`keydown`,e=>{e.key===`Enter`&&lr()}),a.addEventListener(`input`,sr));let o=document.getElementById(`games-search`);o&&o.addEventListener(`input`,ki);let s=document.getElementById(`hero-select-search`);s&&s.addEventListener(`input`,yn);let c=document.getElementById(`modal-sort-name`);c&&c.addEventListener(`click`,()=>vn(`name`));let l=document.getElementById(`modal-sort-weight`);l&&l.addEventListener(`click`,()=>vn(`weight`));let u=document.getElementById(`sort-filter-drawer`);u&&u.addEventListener(`click`,e=>{B(e)});let d=document.getElementById(`filter-drawer-left`);d&&d.addEventListener(`click`,e=>{let t=e.target.closest(`.segmented-pill[data-filter]`);if(t){let e=t.getAttribute(`data-filter`);e&&zn(e);return}Bn(e)}),e(`drawer-close`,()=>B(null,!0)),e(`drawer-reset`,Qn),e(`drawer-apply`,$n),e(`filter-drawer-close-btn`,Un),e(`filter-drawer-reset`,Hn),e(`filter-drawer-apply`,Un),e(`addGroupBtn`,bi),e(`saveGroupBtn`,hi);let f=document.getElementById(`cancelGroupBtn`);f&&f.addEventListener(`click`,async()=>{document.getElementById(`groupName`).value&&!await k(`Discard Changes`,`Discard unsaved changes?`)||yi()}),e(`addHeroBtn`,ci),e(`saveBtn`,ii);let p=document.getElementById(`cancelHeroBtn`);p&&p.addEventListener(`click`,async()=>{document.getElementById(`charName`).value&&!await k(`Discard Changes`,`Discard unsaved changes?`)||si()}),document.querySelectorAll(`#adminSection .panel-header`).forEach(e=>{e.addEventListener(`click`,t=>{let n=e.getAttribute(`data-panel`);n&&Qr(t,n)})});let m=document.getElementById(`login-form`);m&&m.addEventListener(`submit`,e=>{e.preventDefault(),Re()});let h=document.getElementById(`update-password-form`);h&&h.addEventListener(`submit`,e=>{e.preventDefault(),He()});let g=document.getElementById(`auth-btn`);g&&g.addEventListener(`click`,()=>{E(`currentUser`)?Ue():(Le(),Pe())});let _=document.getElementById(`confirm-winner-btn`);_&&_.addEventListener(`click`,()=>{let e=_.getAttribute(`data-game-id`);e&&Fi(e)});let v=document.getElementById(`sort-dropdown-menu`);v&&v.addEventListener(`click`,e=>{let t=e.target.closest(`[data-action="select-sort"]`);t&&or(t.getAttribute(`data-sort-key`),t.getAttribute(`data-sort-asc`)===`true`)});let y=document.getElementById(`filter-drawer-left`);y&&y.addEventListener(`change`,e=>{let t=e.target.closest(`input[type="checkbox"][data-type]`);t&&Vn(t)});let b=document.getElementById(`drawer-body-content`);b&&(b.addEventListener(`click`,e=>{let t=e.target,n=t.closest(`[data-action="toggle-drawer-player-filter"]`);if(n){Yn(parseInt(n.getAttribute(`data-player-idx`),10));return}let r=t.closest(`[data-action="toggle-staged-player-game-filter"]`);if(r){Gn(parseInt(r.getAttribute(`data-player-idx`),10));return}let i=t.closest(`[data-action="drawer-sort-player-change"]`);if(i){Jn(parseInt(i.getAttribute(`data-player-idx`),10));return}let a=t.closest(`[data-action="toggle-drawer-level"]`);if(a){if(a.getAttribute(`data-disabled`)===`true`)return;let e=a.getAttribute(`data-level`);Xn(e===`all`?`all`:parseInt(e,10));return}let o=t.closest(`[data-action="toggle-drawer-group"]`);if(o){if(o.getAttribute(`data-disabled`)===`true`)return;Zn(o.getAttribute(`data-group-id`));return}}),b.addEventListener(`change`,e=>{let t=e.target,n=t.closest(`[data-action="toggle-use-historical"]`);if(n){Kn(n.checked);return}let r=t.closest(`[data-action="toggle-staged-winner-only"]`);if(r){In(r.checked);return}let i=t.closest(`[data-action="drawer-sort-type-change"]`);if(i){qn(i.value);return}let a=t.closest(`[data-action="toggle-staged-ban"]`);if(a){En(a.getAttribute(`data-hero-id`));return}}),b.addEventListener(`input`,e=>{let t=e.target.closest(`[data-action="ban-search-input"]`);t&&Dn(t.value)}));let x=document.getElementById(`active-filters-container`);x&&x.addEventListener(`click`,e=>{let t=e.target;if(t.closest(`[data-action="clear-search-filter"]`)){fr();return}let n=t.closest(`[data-action="remove-filter-chip"]`);if(n){let e=n.getAttribute(`data-type`),t=n.getAttribute(`data-value`);e===`complexity`&&(t=parseInt(t,10)),dr(e,t);return}});let S=document.getElementById(`results`);S&&S.addEventListener(`click`,e=>{let t=e.target,n=t.closest(`[data-action="open-hero-select"]`);if(n){gn(parseInt(n.getAttribute(`data-player-idx`),10));return}let r=t.closest(`[data-action="select-draft-candidate"]`);if(r){Pn(parseInt(r.getAttribute(`data-player-idx`),10),r.getAttribute(`data-hero-id`));return}if(t.closest(`[data-action="cancel-roll"]`)){Tn();return}let i=t.closest(`[data-action="confirm-draft"]`);if(i){Fn(parseInt(i.getAttribute(`data-player-idx`),10));return}});let ee=document.getElementById(`hero-select-options-container`);ee&&ee.addEventListener(`click`,e=>{let t=e.target.closest(`[data-action="select-hero-option"]`);t&&bn(t.getAttribute(`data-hero-name`))});let te=document.getElementById(`heroContainer`);te&&te.addEventListener(`click`,e=>{let t=e.target.closest(`[data-action="toggle-hero-panel"]`);if(t){if(e.target.closest(`a`)||e.target.closest(`.complexity-dice-bar`))return;$r(t)}});let ne=document.getElementById(`groupsListContainer`);ne&&ne.addEventListener(`click`,e=>{let t=e.target,n=t.getAttribute(`data-group-id`);t.closest(`[data-action="edit-group"]`)?gi(n):t.closest(`[data-action="delete-group"]`)?Oi(n):t.closest(`[data-action="save-group-inline"]`)?vi(n):t.closest(`[data-action="cancel-group-edit"]`)&&_i(n)});let C=document.getElementById(`heroesListContainer`);C&&C.addEventListener(`click`,e=>{let t=e.target;t.closest(`[data-action="edit-hero"]`)?ai(parseInt(t.getAttribute(`data-hero-idx`),10)):t.closest(`[data-action="delete-hero"]`)?mi(t.getAttribute(`data-hero-id`)):t.closest(`[data-action="save-hero-inline"]`)?pi(t.getAttribute(`data-hero-id`),parseInt(t.getAttribute(`data-hero-idx`),10)):t.closest(`[data-action="cancel-hero-edit"]`)&&fi()});let w=document.getElementById(`playersListContainer`);w&&(w.addEventListener(`click`,e=>{let t=e.target,n=t.getAttribute(`data-player-id`);t.closest(`[data-action="edit-player"]`)?Si(n):t.closest(`[data-action="save-player-inline"]`)?wi(n):t.closest(`[data-action="cancel-player-edit"]`)&&Ci(n)}),w.addEventListener(`change`,e=>{let t=e.target.closest(`[data-action="player-color-change"]`);t&&Ui(t.getAttribute(`data-player-id`),t)}));let T=document.getElementById(`usersListContainer`);T&&T.addEventListener(`change`,e=>{e.target.closest(`[data-action="user-role-change"]`)&&(alert(`User roles must be modified directly in the Supabase Dashboard for security reasons.`),Ti())});let re=e=>{e&&(e.addEventListener(`click`,e=>{let t=e.target,n=t.closest(`[data-action="toggle-hero-owned"]`);if(n){ni(n.getAttribute(`data-hero-id`),n.getAttribute(`data-selected`)!==`true`);return}let r=t.closest(`[data-action="toggle-collection-group"]`);if(r){if(e.target.closest(`input[type="checkbox"]`)||e.target.closest(`label`))return;ei(r.getAttribute(`data-group-id`),e);return}}),e.addEventListener(`change`,e=>{let t=e.target,n=t.closest(`[data-action="toggle-group-owned"]`);if(n){ri(n.getAttribute(`data-group-id`),n.checked);return}let r=t.closest(`[data-action="toggle-user-hero-owned"]`);if(r){Di(r.getAttribute(`data-user-id`),r.getAttribute(`data-hero-id`),r.checked);return}}))};re(document.getElementById(`collectionsListContainer`)),re(document.getElementById(`collectionContainer`));let ie=document.getElementById(`gamesSection`);ie&&ie.addEventListener(`click`,e=>{let t=e.target;if(t.closest(`[data-action="toggle-history-view-style"]`)){Mi();return}let n=t.closest(`[data-action="toggle-game-expansion"]`);if(n){ji(n.getAttribute(`data-game-id`));return}let r=t.closest(`[data-action="select-winner"]`);if(r){Ni(r.getAttribute(`data-game-id`));return}let i=t.closest(`[data-action="delete-game"]`);if(i){Ii(i.getAttribute(`data-game-id`));return}let a=t.closest(`[data-action="open-winner-modal"]`);if(a){Ni(a.getAttribute(`data-game-id`));return}});let ae=document.getElementById(`winner-selection-container`);ae&&ae.addEventListener(`click`,e=>{let t=e.target.closest(`[data-action="winner-card-click"]`);if(t){let e=t.querySelector(`input[name="winner-selection"]`);if(e){e.checked=!0,ae.querySelectorAll(`.winner-card, .winner-draw-card`).forEach(e=>{e.classList.toggle(`selected`,e===t)});let n=document.getElementById(`confirm-winner-btn`);n&&(n.disabled=!1)}}}),window.addEventListener(`click`,e=>{let t=document.getElementById(`changelog-modal`),n=document.getElementById(`login-modal`),r=document.getElementById(`whats-new-modal`),i=document.getElementById(`update-password-modal`),a=document.getElementById(`hero-select-modal`),o=document.getElementById(`account-modal`);e.target===t&&Jr(),e.target===n&&Fe(),e.target===r&&Xr(),e.target===i&&Ve(),e.target===a&&_n(),e.target===o&&Le();let s=document.getElementById(`sort-dropdown-menu`),c=document.getElementById(`sort-dropdown-container`);s&&s.classList.contains(`show`)&&c&&!c.contains(e.target)&&ar()}),document.addEventListener(`keydown`,e=>{e.key===`Escape`&&_n()})}async function $(){D(`draftModeEnabled`,localStorage.getItem(`draftModeEnabled`)===`true`);let e=parseInt(localStorage.getItem(`draftCount`)||`3`,10);[2,3,4,5].includes(e)||(e=3),D(`draftCount`,e);let t=localStorage.getItem(`bannedHeroIds`);D(`bannedHeroIds`,t?new Set(JSON.parse(t)):new Set),On();let{data:n,error:r}=await m();if(!r&&n){D(`groups`,n),li(),ui();let e=new Set(n.map(e=>e.id));if(E(`activeGroups`).size===0)n.forEach(e=>O(`activeGroups`,`add`,e.id));else for(let t of E(`activeGroups`))e.has(t)||O(`activeGroups`,`delete`,t);er(),ur()}let{data:i,error:a}=await h();if(!a&&i){D(`players`,i),i.forEach(e=>{e.player_color&&Vi(e.id,zi(e.player_color))});let e=i.map(e=>e.name),t=-1,n=E(`currentUser`);i.forEach((r,i)=>{i<6&&(e[i]=r.name,n&&r.user_id===n.id&&(t=i))}),D(`NAMES`,e),D(`loggedInPlayerIndex`,t),Me(),Ne(),at()}let{data:o,error:s}=await g();if(s)return console.error(`Error fetching heroes:`,s);D(`characters`,o.map(e=>{let t=e.user_heroes?.find(e=>e.user_id===E(`currentUser`)?.id),n=t?t.is_owned:!0,r={id:e.id,name:e.name,slug:e.slug,complexity:e.complexity,group_id:e.group_id,is_owned:n,group:e.groups?.name||`Unknown`,weights:[,,,,].fill(250),playCount:[,,,,].fill(0),lastPlayed:[,,,,].fill(`Never`),winCount:[,,,,].fill(0)};return e.player_hero_stats?.forEach(e=>{let t=parseInt(e.player_id.substring(1))-1;t>=0&&t<4&&(r.weights[t]=e.weight)}),r}));let{data:c,error:l}=await _();l?console.error(`Error fetching games:`,l):D(`games`,c.map(e=>({...e,game_players:(e.game_players||[]).slice().sort((e,t)=>parseInt(e.player_id?.substring(1)||`0`,10)-parseInt(t.player_id?.substring(1)||`0`,10))}))),Kr(),ui(),G(),di(),xi(),Ti(),K()&&Ei(),W();let u=E(`currentSort`);D(`currentSort`,null),nr(u)}async function Ji(){try{let{data:{session:e}}=await f();D(`currentUser`,e?.user||null),Me(),await $(),D(`cachedChangelog`,await(await fetch(`changelog.json`)).json());let t=E(`cachedChangelog`);if(t&&t.length>0){let e=t[0],n=document.getElementById(`version-number`);n&&(n.innerText=e.version);let r=localStorage.getItem(`lastSeenVersion`);if(r!==e.version){let n=t.findIndex(e=>e.version===r);Yr(n===-1?[e]:t.slice(0,n))}}le($),fe((e,t)=>{console.log(`[stateStore] Update: ${e} =`,t)})}catch(e){console.error(`Could not load version number:`,e);let t=document.getElementById(`version-number`);t&&(t.innerText=`Error`)}finally{Yi()}}function Yi(){let e=document.getElementById(`preloader`);e&&(e.classList.add(`fade-out`),document.body.classList.add(`loaded`),e.addEventListener(`animationend`,()=>e.remove(),{once:!0}))}window.addEventListener(`DOMContentLoaded`,()=>{qi(),Ji(),p((e,t)=>{D(`currentUser`,t?.user||null),(e===`SIGNED_IN`||e===`SIGNED_OUT`)&&$(),e===`PASSWORD_RECOVERY`&&Be(),Me(),e===`SIGNED_IN`&&Fe()})});